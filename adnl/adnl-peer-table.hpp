...
struct AdnlPeerTableMetrics {
  struct Transport {
    metrics::Counter<"inbound_packets"> inbound_packets;
    metrics::Counter<"decrypt_packets"> decrypt_packets;
    metrics::Counter<"decrypt_bytes"> decrypt_bytes;
    metrics::Labeled<metrics::Counter<"dropped">, metrics::Direction, metrics::Reason> dropped;
    metrics::Gauge<"local_ids"> local_ids;
    metrics::Gauge<"peers"> peers;
    metrics::Gauge<"peer_pairs"> peer_pairs;
    metrics::Gauge<"channels"> channels;
    metrics::Gauge<"static_nodes"> static_nodes;
    void collect(td::Badge<metrics::Context>, metrics::Context ctx) const {
      ctx.collect(inbound_packets);
      ctx.collect(decrypt_packets);
      ctx.collect(decrypt_bytes);
      ctx.collect(dropped);
      ctx.collect(local_ids);
      ctx.collect(peers);
      ctx.collect(peer_pairs);
      ctx.collect(channels);
      ctx.collect(static_nodes);
    }
  };
