...
- **Prefix unused parameters with `_`** (e.g., `_idx`, `_ti`) to avoid `reportUnusedParameter`. **Exceptions:** (1) overrides — when implementing an `@override` method or satisfying a Protocol, basedpyright already knows the signature is dictated by the base, so unused params don't trigger `reportUnusedParameter`. Use the base's parameter names verbatim, no underscore prefix and no `_ = arg` discard. (2) If a *private* function genuinely doesn't use a parameter, prefer deleting the parameter or inlining the function as a lambda over keeping a `_`-prefixed unused one.
