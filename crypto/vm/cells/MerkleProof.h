...
class MerkleProof {
 public:
  using IsPrunedFunction = std::function<bool(const Ref<Cell> &)>;

  // works with proofs wrapped in MerkleProof special cell
  // cells must have zero level
  static Ref<Cell> generate(Ref<Cell> cell, IsPrunedFunction is_pruned);
  ...
  static Ref<Cell> generate_raw(Ref<Cell> cell, IsPrunedFunction is_pruned);
