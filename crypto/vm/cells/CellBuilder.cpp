...
Ref<DataCell> CellBuilder::finalize_copy(bool special) const {
  ...
  auto res = DataCell::create(td::Slice{data, 128}, size(), td::span(refs.data(), size_refs()), special);
  if (res.is_error()) {
    LOG(DEBUG) << res.error();
...
td::Result<Ref<DataCell>> CellBuilder::finalize_novm_nothrow(bool special) {
  auto res = DataCell::create(td::Slice{data, 128}, size(), td::mutable_span(refs.data(), size_refs()), special);
  bits = refs_cnt = 0;
  return res;
}
