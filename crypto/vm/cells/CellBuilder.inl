...
/// 7,558,000
inline td::Result<Ref<DataCell>> CellBuilder::finalize_novm_nothrow(bool special) {
  auto res = DataCell::create(data, size(), td::mutable_span(refs.data(), size_refs()), special);
  bits = refs_cnt = 0;
  return res;
}

inline Ref<DataCell> CellBuilder::finalize_novm(bool special) {
...
inline Ref<DataCell> CellBuilder::finalize(bool special) {
  auto* vm_state_interface = VmStateInterface::get();
  if (!vm_state_interface) {
...
inline Ref<DataCell> CellBuilder::do_create_pruned_branch(Ref<Cell> cell, td::uint32 new_level, td::uint32 virt_level) {
...
inline Ref<DataCell> CellBuilder::create_merkle_proof(Ref<Cell> cell_proof) {
  CellBuilder cb;
...
inline Ref<DataCell> CellBuilder::create_merkle_update(Ref<Cell> from_proof, Ref<Cell> to_proof) {
  CellBuilder cb;
...
inline void CellBuilder::reset(void) {
  while (refs_cnt > 0) {
    refs[--refs_cnt].clear();
  }
  bits = 0;
...
inline CellBuilder& CellBuilder::operator=(const CellBuilder& other) {
  bits = other.bits;
...
inline CellBuilder& CellBuilder::operator=(CellBuilder&& other) {
  bits = other.bits;
  refs_cnt = other.refs_cnt;
...
inline bool CellBuilder::can_extend_by(std::size_t new_bits, unsigned new_refs) const {
  return (new_bits <= Cell::max_bits - bits && new_refs <= (unsigned)Cell::max_refs - refs_cnt);
}

inline bool CellBuilder::can_extend_by(std::size_t new_bits) const {
  return new_bits <= Cell::max_bits - bits;
}

inline CellBuilder& CellBuilder::store_bytes(const unsigned char* str, std::size_t len) {
  ensure_throw(len <= Cell::max_bytes);
  return store_bits(str, len * 8);
}

inline CellBuilder& CellBuilder::store_bytes(const unsigned char* str, const unsigned char* end) {
  ensure_throw(end >= str && end <= str + Cell::max_bytes);
  return store_bits(str, (end - str) * 8);
}

inline CellBuilder& CellBuilder::store_bytes(const char* str, std::size_t len) {
  return store_bytes((const unsigned char*)(str), len);
}

inline CellBuilder& CellBuilder::store_bytes(const char* str, const char* end) {
  return store_bytes((const unsigned char*)(str), (const unsigned char*)(end));
}

inline CellBuilder& CellBuilder::store_bytes(td::Slice s) {
  return store_bytes((const unsigned char*)(s.data()), (const unsigned char*)(s.data() + s.size()));
}

inline bool CellBuilder::store_bytes_bool(const unsigned char* str, std::size_t len) {
  return len <= Cell::max_bytes && store_bits_bool(str, len * 8);
}

inline bool CellBuilder::store_bytes_bool(const char* str, std::size_t len) {
  return len <= Cell::max_bytes && store_bits_bool((const unsigned char*)str, len * 8);
}

inline bool CellBuilder::store_bytes_bool(td::Slice s) {
  return store_bytes_bool((const unsigned char*)s.data(), s.size());
}

inline bool CellBuilder::store_bits_bool(const unsigned char* str, std::size_t bit_count, int bit_offset) {
  unsigned pos = bits;
...
inline CellBuilder& CellBuilder::store_bits(const unsigned char* str, std::size_t bit_count, int bit_offset) {
  unsigned pos = bits;
...
inline CellBuilder& CellBuilder::store_bits(const td::BitSlice& bs) {
  return store_bits(bs.get_ptr(), bs.size(), bs.get_offs());
}

inline CellBuilder& CellBuilder::store_bits(const char* str, std::size_t bit_count, int bit_offset) {
  return store_bits((const unsigned char*)str, bit_count, bit_offset);
}

inline CellBuilder& CellBuilder::store_bits(td::ConstBitPtr bs, std::size_t bit_count) {
  return store_bits(bs.ptr, bit_count, bs.offs);
}

inline bool CellBuilder::store_bits_bool(td::ConstBitPtr bs, std::size_t bit_count) {
  return store_bits_bool(bs.ptr, bit_count, bs.offs);
}

inline CellBuilder& CellBuilder::store_bits_same(std::size_t bit_count, bool val) {
  unsigned pos = bits;
  if (prepare_reserve(bit_count)) {
...
inline bool CellBuilder::store_bits_same_bool(std::size_t bit_count, bool val) {
  unsigned pos = bits;
  if (!prepare_reserve(bit_count)) {
    return false;
  }
...
inline td::BitSliceWrite CellBuilder::reserve_slice(std::size_t bit_count) {
  unsigned offs = bits;
  if (prepare_reserve(bit_count)) {
...
inline CellBuilder& CellBuilder::reserve_slice(std::size_t bit_count, td::BitSliceWrite& bsw) {
  unsigned offs = bits;
  if (prepare_reserve(bit_count)) {
...
inline bool CellBuilder::store_bool_bool(bool val) {
...
inline bool CellBuilder::store_long_bool(long long val, unsigned val_bits) {
...
inline bool CellBuilder::store_long_rchk_bool(long long val, unsigned val_bits) {
  if (val_bits > 64 || !can_extend_by(val_bits)) {
    return false;
  }
...
inline bool CellBuilder::store_ulong_rchk_bool(unsigned long long val, unsigned val_bits) {
  if (val_bits > 64 || !can_extend_by(val_bits)) {
    return false;
  }
...
inline CellBuilder& CellBuilder::store_long(long long val, unsigned val_bits) {
  return store_long_top(val_bits == 0 ? 0 : (unsigned long long)val << (64 - val_bits), val_bits);
}

inline CellBuilder& CellBuilder::store_long_top(unsigned long long val, unsigned top_bits) {
  unsigned pos = bits;
...
inline bool CellBuilder::store_uint_less(unsigned upper_bound, unsigned long long val) {
  return val < upper_bound && store_long_bool(val, 32 - td::count_leading_zeroes32(upper_bound - 1));
}

inline bool CellBuilder::store_uint_leq(unsigned upper_bound, unsigned long long val) {
  return val <= upper_bound && store_long_bool(val, 32 - td::count_leading_zeroes32(upper_bound));
}

inline bool CellBuilder::store_int256_bool(const td::BigInt256& val, unsigned val_bits, bool sgnd) {
  unsigned pos = bits;
...
inline CellBuilder& CellBuilder::store_int256(const td::BigInt256& val, unsigned val_bits, bool sgnd) {
  return ensure_pass(store_int256_bool(val, val_bits, sgnd));
}

inline bool CellBuilder::store_int256_bool(td::RefInt256 val, unsigned val_bits, bool sgnd) {
  return val.not_null() && store_int256_bool(*val, val_bits, sgnd);
}

inline bool CellBuilder::store_builder_ref_bool(vm::CellBuilder&& cb) {
  return store_ref_bool(cb.finalize());
}

inline bool CellBuilder::store_ref_bool(Ref<Cell> ref) {
...
inline CellBuilder& CellBuilder::store_ref(Ref<Cell> ref) {
  return ensure_pass(store_ref_bool(std::move(ref)));
}

inline td::uint16 CellBuilder::get_depth() const {
  int d = 0;
...
inline bool CellBuilder::append_data_cell_bool(const DataCell& cell) {
...
inline CellBuilder& CellBuilder::append_data_cell(const DataCell& cell) {
  return ensure_pass(append_data_cell_bool(cell));
}

inline bool CellBuilder::append_data_cell_bool(Ref<DataCell> cell_ref) {
  return append_data_cell_bool(*cell_ref);
}

inline CellBuilder& CellBuilder::append_data_cell(Ref<DataCell> cell_ref) {
  return ensure_pass(append_data_cell_bool(std::move(cell_ref)));
}

inline bool CellBuilder::append_builder_bool(const CellBuilder& cb) {
...
inline CellBuilder& CellBuilder::append_builder(const CellBuilder& cb) {
  return ensure_pass(append_builder_bool(cb));
}

inline bool CellBuilder::append_builder_bool(Ref<CellBuilder> cb_ref) {
  return append_builder_bool(*cb_ref);
}

inline CellBuilder& CellBuilder::append_builder(Ref<CellBuilder> cb_ref) {
  return ensure_pass(append_builder_bool(std::move(cb_ref)));
}

inline bool CellBuilder::append_bitstring(const td::BitString& bs) {
  return store_bits_bool(bs.cbits(), bs.size());
}

inline bool CellBuilder::append_bitstring(Ref<td::BitString> bs_ref) {
  return bs_ref.not_null() && append_bitstring(*bs_ref);
}

inline bool CellBuilder::append_bitstring_chk(const td::BitString& bs, unsigned size) {
  return bs.size() == size && store_bits_bool(bs.cbits(), size);
}

inline bool CellBuilder::append_bitstring_chk(Ref<td::BitString> bs_ref, unsigned size) {
  return bs_ref.not_null() && append_bitstring_chk(*bs_ref, size);
}

inline bool CellBuilder::append_bitslice(const td::BitSlice& bs) {
  return store_bits_bool(bs.bits(), bs.size());
}

inline bool CellBuilder::store_maybe_ref(Ref<Cell> cell) {
  if (cell.is_null()) {
...
inline void CellBuilder::flush(unsigned char d[2]) const {
...
inline const unsigned char* CellBuilder::compute_hash(unsigned char buffer[Cell::hash_bytes]) const {
  unsigned char tmp[2];
  flush(tmp);
...
inline int CellBuilder::serialize(unsigned char* buff, int buff_size) const {
...
inline CellBuilder* CellBuilder::make_copy() const {
...
inline std::ostream& operator<<(std::ostream& os, const CellBuilder& cb) {
  return os << cb.to_hex();
}
}  // namespace vm
