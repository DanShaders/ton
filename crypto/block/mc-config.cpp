...
td::Result<std::unique_ptr<ValidatorSet>> Config::unpack_validator_set(Ref<vm::Cell> vset_root) {
  ...
  // LOG(ERROR) << "Records " << rec.total;
  //
  //
  td::Status status;
  auto check_func = [&](Ref<vm::CellSlice> value, td::ConstBitPtr key, int key_len) {
      if (value.is_null()) {
        // never happens in tests // LOG(ERROR) << "Not consecutive indices";
        status = td::Status::Error("indices in a validator set dictionary must be integers 0..total-1");
        return false;
      }

      gen::ValidatorDescr::Record_validator_addr descr;
      if (!tlb::csr_unpack(value, descr)) {
        descr.adnl_addr.set_zero();
        if (!(gen::t_ValidatorDescr.unpack_validator(value.write(), descr.public_key, descr.weight) &&
              value->empty_ext())) {
          status = td::Status::Error(PSLICE() << "validator #" << key.get_int(key_len)
      ...
      if (!descr.weight) {
        status = td::Status::Error(PSLICE() << "validator #" << key.get_int(key_len) << " has zero weight");
        return false;
      }
      if (descr.weight > ~(ptr->total_weight)) {
        status = td::Status::Error("total weight of all validators in validator set exceeds 2^64");
        return false;
      }
  ...
  if (!dict.check_for_each(check_func)) {
      return status;
  }
