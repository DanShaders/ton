...
td::Result<tonlib::FFILiteClient> create_ffi_lite_client(TonlibEventLoop *loop, const char *config) {
  ...
  TRY_RESULT(address_value, object.extract_required_field("address", td::JsonValue::Type::String));
  td::IPAddress parsed_address;
  TRY_STATUS(parsed_address.init_host_port(address_value.get_string().str()));

  TRY_RESULT(key_value, object.extract_required_field("server_public_key", td::JsonValue::Type::Object));
  ton::tl_object_ptr<ton::ton_api::PublicKey> public_key_tl;
  TRY_STATUS(from_json(public_key_tl, std::move(key_value)));
  if (!public_key_tl) {
    return td::Status::Error("server_public_key is required in config");
  }
  auto server_public_key_slice = ton::serialize_tl_object(public_key_tl.get(), true);
