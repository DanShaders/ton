...
// ===== Event Loop =====
// An interaction with the new interface of tonlib starts by creating a TonlibEventLoop object that
// allows foreign caller to wait for asynchronous event completion in a mostly non-blocking manner.
//
...
//    continuations of resolved awaitables.
...
//
// Non-destroyed instances of actors will deadlock the function. (Calling `tonlib_{actor}_destroy`
// during `tonlib_event_loop_destroy` is UB as it violates the global happens-before ordering
// requirement.)
...
// ===== Response =====
// TonlibResponse is an awaitable that resolves with the result of a query. Instance of it can
// usually be obtained from functions on actors that do async requests.
...
// Returns true if the response is an error. Can only be called on resolved response.
...
// Returns the response. Can only be called on resolved response if `tonlib_response_is_error` is
// false.
