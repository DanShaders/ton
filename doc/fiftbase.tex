...
In the future, we will assume that the Fift assembler is already loaded and omit the phrase {\tt "Asm.fif" include} from our examples.
...
\item {\tt \$pos} ($S$ $S'$ -- $x$ or $-1$), returns the position (byte offset)~$x$ of the first occurrence of substring $S'$ in string~$S$ or $-1$.
