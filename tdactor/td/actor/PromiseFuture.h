...
template <typename T, typename Arg>
concept IsPromiseInterfacePtr =
    IsSpecializationOf<T, std::unique_ptr> && IsPromiseInterface<typename T::element_type, Arg>;

template <typename F, typename T>
concept GoodExplicitLambda = requires(std::remove_reference_t<F> f, td::Result<T> result) { f(std::move(result)); };

template <typename RawF>
struct GoodImplicitLambdaHelper {
  using F = std::remove_reference_t<RawF>;
...
auto lambda_promise(FMaybeRef &&f) {
  using F = std::remove_reference_t<FMaybeRef>;
...
auto lambda_promise(F &&f) {
  return LambdaPromise<T, std::remove_reference_t<F>>(std::forward<F>(f));
}
...
auto promise_interface(F &&f) {
  return lambda_promise<T, F>(std::forward<F>(f));
}
...
class Promise final {
 public:
  ...
  Promise(Interface &&interface) : promise_(std::make_unique<Interface>(std::move(interface))) {
  }

  template <detail::GoodExplicitLambda<T> F>
  Promise(F &&f) : promise_(std::make_unique<LambdaPromise<T, F>>(std::forward<F>(f))) {
  }
