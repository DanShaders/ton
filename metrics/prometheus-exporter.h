...
class PrometheusExporter final : public td::actor::Actor {
 ...
 private:
  ...
  td::actor::Task<> collect_and_respond(PayloadPtr payload, double started_at);

  struct Stats {
    metrics::Gauge<"collectors"> collectors;
    metrics::Counter<"collections"> collections;
    metrics::Gauge<"last_collection_duration_seconds"> last_collection_duration;
    metrics::Gauge<"last_collection_timestamp_seconds"> last_collection_timestamp;

    void collect(td::Badge<metrics::Context>, metrics::Context ctx) const {
      ctx.collect(collectors);
      ctx.collect(collections);
      ctx.collect(last_collection_duration);
      ctx.collect(last_collection_timestamp);
    }
  };

  metrics::Prefixed<"exporter", Stats> stats_;
