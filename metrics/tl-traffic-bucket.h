...
//
// Collectable leaf emitting two families: `bytes_total{tl}` and `messages_total{tl}`. Wrap in
// Labeled<TlTrafficBucket, ...> to add outer labels (e.g. kind, direction).
class TlTrafficBucket {
 public:
  void account(td::Slice payload);
  void account(td::int32 magic, td::uint64 size);

  // Cell-wise merge of another bucket's counts — lets a bucket accumulated on one actor be drained
  // into an aggregate on another (see App::operator+=).
