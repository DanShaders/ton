...
async def running(
    ...
    async with Network(install, working_dir) as network:
        ...
        await asyncio.wait_for(balance_changed(), timeout=60)
...
async def test_main_wallet_seqno_after_deploy(running: _RunningBasic):
    state = await running.main_wallet.current
