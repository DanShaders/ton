...
class WatchBus(Generic[_TRes]):
    ...
    def __init__(self, kind: str, *, queue_size: int = DEFAULT_QUEUE_SIZE):
        self._kind: str = kind
        self._inner: BroadcastQueue[WatchEvent[_TRes]] = BroadcastQueue(kind, queue_size=queue_size)
    ...
    @asynccontextmanager
    async def subscribe(self) -> AsyncGenerator[AsyncIterator[WatchEvent[_TRes]]]:
...
async def _wrap_overflow(
    events: AsyncIterator[WatchEvent[_TRes]],
    kind: str,
