...
namespace {
using namespace ton::metrics;

// Minimal Collectable leaf so we can build Labeled<...> with values we can read back via at().
struct Cell {
  int v = 0;
  void collect(td::Badge<Context>, Context) const {
  }
};
}  // namespace
