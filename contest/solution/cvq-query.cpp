...
bool ContestValidateQuery::top_level_reject_query(std::string error, td::BufferSlice reason) {
  ...
  //   LOG(ERROR) << "Test index #" << testIndex << " will TOP LEVEL throw from thread: " << render_thread_id(std::this_thread::get_id()); // !TEMP_THREAD
  //   throw error;
...
void ContestValidateQuery::finish_query() {
  ...
  if (__pending_unpack_block_data != 0) LOG(ERROR) << "Generated atomic variable should be exactly 0, when reaching 'finish_query', but variable __pending_unpack_block_data ended up as: " << __pending_unpack_block_data;
  if (__pending_precheck_account_transactions != 0) LOG(ERROR) << "Generated atomic variable should be exactly 0, when reaching 'finish_query', but variable __pending_precheck_account_transactions ended up as: " << __pending_precheck_account_transactions;
  ...
  if (__pending_check_new_state != 0) LOG(ERROR) << "Generated atomic variable should be exactly 0, when reaching 'finish_query', but variable __pending_check_new_state ended up as: " << __pending_check_new_state;
