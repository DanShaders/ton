...
/**
 * Rejects the validation and logs an error message.
 * stop() call removed for multi-thread not to access thread-local variable
 *
 * @param error The error message to be logged.
 * @param reason The reason for rejecting the validation.
 *
 * @returns False indicating that the validation failed.
 */
bool ContestValidateQuery::reject_query_without_stop(std::string error, td::BufferSlice reason) {
  error = error_ctx() + error;
  LOG(WARNING) << "REJECT-noStop: aborting validation of block candidate for : " << error;
 return false;
}
...
bool ContestValidateQuery::fatal_error_without_stop(std::string err_msg, int err_code) {
  return fatal_error(td::Status::Error(err_code, error_ctx() + err_msg));
}
...
std::unique_ptr<block::Account> ContestValidateQuery::unpack_account(td::ConstBitPtr addr) {
  std::lock_guard<std::mutex> lock(ps_account_dict_mutex);   // protect update ps_.account_dict_ for multi-thread  
  auto dict_entry = ps_.account_dict_->lookup_extra(addr, 256);
  auto new_acc = make_account_from(addr, std::move(dict_entry.first));
  if (!new_acc) {
    reject_query_without_stop("cannot load state of account "s + addr.to_hex(256) + " from previous shardchain state");
    return {};
  }
  if (!new_acc->belongs_to_shard(shard_)) {
    reject_query_without_stop(PSTRING() << "old state of account " << addr.to_hex(256)
...
bool ContestValidateQuery::check_one_transaction(block::Account& account, ton::LogicalTime lt, Ref<vm::Cell> trans_root,
  ...
  if (in_msg_root.not_null()) {
    ...
    if (in_descr_cs.is_null()) {
      return reject_query_without_stop(PSTRING() << "inbound message with hash " << in_msg_root->get_hash().to_hex()
    ...
    if (in_msg_tag == block::gen::InMsg::msg_import_ext) {
      ...
      if (info.created_lt >= lt) {
        return reject_query_without_stop(PSTRING() << "transaction " << lt << " of " << addr.to_hex()
      ...
      if (in_msg_tag == block::gen::InMsg::msg_import_imm || in_msg_tag == block::gen::InMsg::msg_import_fin ||
        ...
        if (!block::tlb::unpack_cell(in_descr_cs->prefetch_ref(), msg_env)) {
          return reject_query_without_stop(PSTRING() << "InMsg record for inbound message with hash "
      ...
      if (info.created_lt != start_lt_ || !is_special_tx) {
        std::lock_guard<std::mutex> lock(msg_proc_lt_mutex_);  
    ...
    if (d_wc != workchain() || d_addr != addr) {
      return reject_query_without_stop(PSTRING() << "inbound message of transaction " << lt << " of account " << addr.to_hex()
    ...
    if (in_msg_trans->get_hash() != trans_root->get_hash()) {
      return reject_query_without_stop(PSTRING() << "InMsg record for inbound message with hash " << in_msg_root->get_hash().to_hex()
                                    << " of transaction " << lt << " of account " << addr.to_hex()
  ...
  for (int i = 0; i < trans.outmsg_cnt; i++) {
    ...
    if (out_descr_cs.is_null()) {
      return reject_query_without_stop(PSTRING() << "outbound message #" << i + 1 << " with hash "
    ...
    if (s_wc != workchain() || ss_addr != addr) {
      return reject_query_without_stop(PSTRING() << "outbound message #" << i + 1 << " of transaction " << lt << " of account "
    ...
    if (out_msg_trans->get_hash() != trans_root->get_hash()) {
      return reject_query_without_stop(PSTRING() << "OutMsg record for outbound message #" << i + 1 << " with hash "
                                    << out_msg_root->get_hash().to_hex() << " of transaction " << lt << " of account "
    ...
    if (tag != block::gen::OutMsg::msg_export_ext) {
      ...
      if (account_expected_defer_all_messages_.count(ss_addr) && !is_deferred) {
        return reject_query_without_stop(
      ...
      if (is_deferred) {
        ...
        if (!deferring_messages_enabled_ && !account_expected_defer_all_messages_.count(ss_addr)) {
          return reject_query_without_stop(PSTRING() << "outbound message #" << i + 1 << " on account " << workchain() << ":"
                                        << ss_addr.to_hex() << " is deferred, but deferring messages is disabled");
        }
        if (i == 0 && !account_expected_defer_all_messages_.count(ss_addr)) {
          return reject_query_without_stop(PSTRING() << "outbound message #1 on account " << workchain() << ":" << ss_addr.to_hex()
  ...
  if (tag == block::gen::TransactionDescr::trans_merge_prepare ||
    ...
    if (split && !before_split_) {
      return reject_query_without_stop(PSTRING() << "transaction " << lt << " of account " << addr.to_hex()
  ...
  switch (tag) {
    case block::gen::TransactionDescr::trans_ord: {
      ...
      if (in_msg_root.is_null()) {
        return reject_query_without_stop(PSTRING() << "ordinary transaction " << lt << " of account " << addr.to_hex()
    ...
    case block::gen::TransactionDescr::trans_storage: {
      ...
      if (in_msg_root.not_null()) {
        return reject_query_without_stop(PSTRING() << "storage transaction " << lt << " of account " << addr.to_hex()
      ...
      // FIXME
      return reject_query_without_stop(PSTRING() << "unable to verify storage transaction " << lt << " of account "
                                    << addr.to_hex());
      break;
    }
    case block::gen::TransactionDescr::trans_tick_tock: {
      ...
      if (in_msg_root.not_null()) {
        return reject_query_without_stop(PSTRING() << (is_tock ? "tock" : "tick") << " transaction " << lt << " of account "
    ...
    case block::gen::TransactionDescr::trans_merge_prepare: {
      ...
      if (in_msg_root.not_null()) {
        return reject_query_without_stop(PSTRING() << "merge prepare transaction " << lt << " of account " << addr.to_hex()
                                      << " has an inbound message");
      }
      ...
      // FIXME
      return reject_query_without_stop(PSTRING() << "unable to verify merge prepare transaction " << lt << " of account "
                                    << addr.to_hex());
      break;
    }
    case block::gen::TransactionDescr::trans_merge_install: {
      ...
      if (in_msg_root.is_null()) {
        return reject_query_without_stop(PSTRING() << "merge install transaction " << lt << " of account " << addr.to_hex()
                                      << " has no inbound message");
      }
      need_credit_phase = true;
      // FIXME
      return reject_query_without_stop(PSTRING() << "unable to verify merge install transaction " << lt << " of account "
                                    << addr.to_hex());
      break;
    }
    case block::gen::TransactionDescr::trans_split_prepare: {
      ...
      if (in_msg_root.not_null()) {
        return reject_query_without_stop(PSTRING() << "split prepare transaction " << lt << " of account " << addr.to_hex()
                                      << " has an inbound message");
      }
      if (trans.outmsg_cnt > 1) {
        return reject_query_without_stop(PSTRING() << "split prepare transaction " << lt << " of account " << addr.to_hex()
      ...
      // FIXME
      return reject_query_without_stop(PSTRING() << "unable to verify split prepare transaction " << lt << " of account "
                                    << addr.to_hex());
      break;
    }
    case block::gen::TransactionDescr::trans_split_install: {
      ...
      if (in_msg_root.is_null()) {
        return reject_query_without_stop(PSTRING() << "split install transaction " << lt << " of account " << addr.to_hex()
                                      << " has no inbound message");
      }
      // FIXME
      return reject_query_without_stop(PSTRING() << "unable to verify split install transaction " << lt << " of account "
                                    << addr.to_hex());
      break;
    }
  }
  ...
  if (in_msg_root.not_null()) {
    if (!trs->unpack_input_msg(ihr_delivered, &action_phase_cfg_)) {
      ...
      return reject_query_without_stop(PSTRING() << "could not unpack inbound " << (external ? "external" : "internal")
  ...
  if (trs->bounce_enabled) {
    if (!trs->prepare_storage_phase(storage_phase_cfg_, true)) {
      return reject_query_without_stop(PSTRING() << "cannot re-create storage phase of transaction " << lt << " for smart contract "
                                    << addr.to_hex());
    }
    if (need_credit_phase && !trs->prepare_credit_phase()) {
      return reject_query_without_stop(PSTRING() << "cannot create re-credit phase of transaction " << lt << " for smart contract "
                                    << addr.to_hex());
    }
  } else {
    if (need_credit_phase && !trs->prepare_credit_phase()) {
      return reject_query_without_stop(PSTRING() << "cannot re-create credit phase of transaction " << lt << " for smart contract "
                                    << addr.to_hex());
    }
  ...
  if (!trs->prepare_compute_phase(compute_phase_cfg_)) {
    return reject_query_without_stop(PSTRING() << "cannot re-create compute phase of transaction " << lt << " for smart contract "
                                  << addr.to_hex());
  }
  if (!trs->compute_phase->accepted) {
    if (external) {
      return reject_query_without_stop(PSTRING() << "inbound external message claimed to be processed by ordinary transaction " << lt
      ...
      return reject_query_without_stop(PSTRING() << "inbound internal message processed by ordinary transaction " << lt
  ...
  if (trs->compute_phase->success && !trs->prepare_action_phase(action_phase_cfg_)) {
    return reject_query_without_stop(PSTRING() << "cannot re-create action phase of transaction " << lt << " for smart contract "
                                  << addr.to_hex());
  }
  if (trs->bounce_enabled &&
    ...
    return reject_query_without_stop(PSTRING() << "cannot re-create bounce phase of  transaction " << lt << " for smart contract "
                                  << addr.to_hex());
  }
  if (!trs->serialize()) {
    return reject_query_without_stop(PSTRING() << "cannot re-create the serialization of  transaction " << lt
                                  << " for smart contract " << addr.to_hex());
  }
  if (!trs->update_limits(*block_limit_status_, /* with_gas = */ false, /* with_size = */ false)) {
    return fatal_error_without_stop(PSTRING() << "cannot update block limit status to include transaction " << lt << " of account "
                                 << addr.to_hex());
  }
  ...
  if (total_gas_used_ > block_limits_->gas.hard() + compute_phase_cfg_.gas_limit) {
    return reject_query_without_stop(PSTRING() << "gas block limits are exceeded: total_gas_used > gas_limit_hard + trx_gas_limit ("
  ...
  if (trans_root2.is_null()) {
    return reject_query_without_stop(PSTRING() << "the re-created transaction " << lt << " for smart contract " << addr.to_hex()
  ...
  if (trans_root2->get_hash() != trans_root->get_hash()) {
    ...
    return reject_query_without_stop(PSTRING() << "the transaction " << lt << " of " << addr.to_hex() << " has hash "
  ...
  if (!(tlb::unpack_cell(trans_root2, trans2) &&
    ...
    return fatal_error_without_stop(PSTRING() << "cannot unpack the re-created transaction " << lt << " of " << addr.to_hex());
  }
  if (hash_upd2.old_hash != hash_upd.old_hash) {
    return fatal_error_without_stop(PSTRING() << "the re-created transaction " << lt << " of " << addr.to_hex()
                                 << " is invalid: it starts from account state with different hash");
  }
  if (hash_upd2.new_hash != account.total_state->get_hash().bits()) {
    return fatal_error_without_stop(
...
bool ContestValidateQuery::check_account_transactions(const StdSmcAddress& acc_addr, Ref<vm::CellSlice> acc_blk_root) {
  ...
  if (!account_p) {
    return reject_query_without_stop("cannot unpack old state of account "s + acc_addr.to_hex());
  }
  ...
  if (!trans_dict.check_for_each_extra([this, &account, min_trans_lt, max_trans_lt](Ref<vm::CellSlice> value,
    ...
    return reject_query_without_stop("at least one Transaction of account "s + acc_addr.to_hex() + " is invalid");
  }

  // See Collator::combine_account_trabsactions
  {
  if (account.total_state->get_hash() != account.orig_total_state->get_hash()) {
    std::lock_guard<std::mutex> lock(ns_account_dict_mutex);   // protect update ns_.account_dict_ for multi-thread
    // account changed
    if (account.orig_status == block::Account::acc_nonexist) {
      ...
      if (!(cb.store_ref_bool(account.total_state)             // account_descr$_ account:^Account
        ...
        return fatal_error_without_stop(std::string{"cannot add newly-created account "} + account.addr.to_hex() +
                           " into ShardAccounts");
      }
      ...
      if (ns_.account_dict_->lookup_delete(account.addr).is_null()) {
        return fatal_error_without_stop(std::string{"cannot delete account "} + account.addr.to_hex() + " from ShardAccounts");
      }
    } else {
      ...
      if (!(cb.store_ref_bool(account.total_state)             // account_descr$_ account:^Account
        ...
        return fatal_error_without_stop(std::string{"cannot modify existing account "} + account.addr.to_hex() +
                           " in ShardAccounts");
      }
    }
  }
  ...
  {
    std::lock_guard<std::mutex> lock_ns(ns_account_dict_mutex);   
    std::lock_guard<std::mutex> lock_ps(ps_account_dict_mutex);   
...
bool ContestValidateQuery::check_transactions() {
  ...
  // Group account block entries by account address.
  std::map<td::BitArray<256>,
                     std::vector<Ref<vm::CellSlice>>>
      accountsGroups;
  
  // Traverse the account_blocks_dict_.
  account_blocks_dict_->check_for_each_extra(
      [&accountsGroups](Ref<vm::CellSlice> value, Ref<vm::CellSlice> extra, td::ConstBitPtr key, int key_len) -> bool {
        CHECK(key_len == 256);
        td::BitArray<256> accKey;
        accKey.bits().copy_from(key, 256);
        accountsGroups[accKey].push_back(std::move(value));
        return true; 
      }
  );  

  // Now launch one asynchronous task per group using std::async.
  // Each task processes all account blocks for that account sequentially.
  std::vector<std::future<bool>> futures;
  std::atomic<bool> failed_exit(false);

  for (auto& groupPair : accountsGroups) {

    auto localBlocks = std::move(groupPair.second);
    // groupPair.first is the account address (td::BitArray<256>)
    // groupPair.second is a vector of account block cell slices for that account.
    //TODO: use threadpool for efficiency
    futures.push_back(std::async(std::launch::async, [this,  &failed_exit, accountKey = groupPair.first, localBlocks = std::move(localBlocks)]() -> bool {
      
      // Process all account blocks for this account sequentially.
      for (auto& blockValue : localBlocks) {

          if (failed_exit.load(std::memory_order_acquire)) {
            LOG(INFO)  << "Early exit for account: " << accountKey.to_hex() ;
            return false;
          }  
          // If check_account_transactions fails for any block, return false.
          // (Make sure that check_account_transactions is thread-safe if it updates shared data.)
          if (!this->check_account_transactions(accountKey, std::move(blockValue))) {
            failed_exit.store(true, std::memory_order_release);
            LOG(INFO) << "Failure processing account: " << accountKey.to_hex();
            return false;
          }
      }
      return true;
  ...
  // Wait for all futures to finish and combine their results.
