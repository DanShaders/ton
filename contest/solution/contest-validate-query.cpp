...
ContestValidateQuery::ContestValidateQuery(BlockIdExt block_id, td::BufferSlice block_data,
    ...
    , shard_pfx_len_(ton::shard_prefix_length(shard_)) {
      async_tx_checker = std::make_unique<TransactionChecker>();
}
...
bool ContestValidateQuery::reject_query(std::string error, td::BufferSlice reason) {
  ...
  async_tx_checker->stop_and_wait();
  if (main_promise) {
...
bool ContestValidateQuery::fetch_config_params() {
  ...
  }
  {
    ...
    compute_phase_cfg_defer_out_queue_size_limit = compute_phase_cfg_.size_limits.defer_out_queue_size_limit;
  }
  {
    ...
    action_phase_cfg_fwd_std = block::MsgPrices{rec.lump_price, rec.bit_price, rec.cell_price, rec.ihr_price_factor,
                         (unsigned)rec.first_frac, (unsigned)rec.next_frac};
  }
  {
...
bool ContestValidateQuery::prepare_transaction_checker() {
  LOG(DEBUG) << "prepare_transaction_checker";
  block::gen::Block::Record blk;
  block::gen::BlockExtra::Record extra;
  ...
  auto acc_blocks_cs = vm::load_cell_slice_ref(extra.account_blocks);

  auto in_msg_dict = std::make_unique<vm::AugmentedDictionary>(std::move(inmsg_cs), 256, block::tlb::aug_InMsgDescr);
  auto out_msg_dict = std::make_unique<vm::AugmentedDictionary>(std::move(outmsg_cs), 256, block::tlb::aug_OutMsgDescr);
  auto account_blocks_dict = std::make_unique<vm::AugmentedDictionary>(std::move(acc_blocks_cs), 256, block::tlb::aug_ShardAccountBlocks);

  return async_tx_checker && async_tx_checker->prepare(
      std::move(in_msg_dict), std::move(out_msg_dict), std::move(account_blocks_dict));
}
...
bool ContestValidateQuery::unpack_block_data() {
  ...
  if (!prepare_transaction_checker()){
    return reject_query("Prepare asynch transaction checker failed");
  };
...
bool ContestValidateQuery::unpack_dispatch_queue_update() {
  ...
  try {
    ...
    if (have_out_msg_queue_size_in_state_ &&
        old_out_msg_queue_size_ <= compute_phase_cfg_defer_out_queue_size_limit) {
  ...
  switch (tag) {
    ...
    case block::gen::InMsg::msg_import_tr: {
      ...
      td::RefInt256 transit_fee =
          from_dispatch_queue ? td::zero_refint() : action_phase_cfg_fwd_std.get_next_part(env.fwd_fee_remaining);
...
/**
 * Initializes and starts the transactions checking process.
 * Configures the transaction checker with necessary parameters and launches the verification procedure in a separate thread.
 */
bool ContestValidateQuery::start_async_transactions_check(){
  LOG(INFO) << "start async transaction checking process";
  ns_.account_dict_ = std::make_unique<vm::AugmentedDictionary>(
      ps_.account_dict_->get_root(), 
      256, 
      block::tlb::aug_ShardAccounts
  );
  async_tx_checker->init(
      now_,                                // current time
      start_lt_,                           // logical time
      verbosity,                           // verbosity level
      after_merge_,                        // after merge flag
      before_split_,                       // before split flag
      msg_metadata_enabled_,               // message metadata enabled flag
      deferring_messages_enabled_,         // deferring messages enabled flag
      block_limits_->gas.hard(),           // gas limit hard value
      shard_,                              // shard
      std::move(storage_phase_cfg_),       // storage phase configuration
      std::move(compute_phase_cfg_),       // compute phase configuration
      std::move(block_limit_status_),      // block limit status
      std::move(msg_proc_lt_),             // message processing logical times
      std::move(action_phase_cfg_),        // action phase configuration
      std::move(account_expected_defer_all_messages_), // account messages deferral
      std::move(ns_.account_dict_),        // new state account dictionary
      std::move(ps_.account_dict_),        // prev state account dictionary
      std::move(total_burned_)             // total burned
  );
  return async_tx_checker->run();
}

/**
 * Waits for the transactions checking thread to complete and retrieves the results.
 * Transfers intermediate computation results from the transaction checker to the current instance.
 *
 * @return True if the transactions check was successful, False otherwise.
 */
bool ContestValidateQuery::wait_for_async_transactions_check() {
    LOG(INFO) << "wait for async transaction checking process complate";
    // wait for the completion of the transaction check
    bool res = async_tx_checker->wait();
    LOG(INFO) << "async transaction checking complate";
    // account dictionaries
    ps_.account_dict_ = std::move(async_tx_checker->ps_account_dict_);
    ns_.account_dict_ = std::move(async_tx_checker->ns_account_dict_);

    // configuration settings
    storage_phase_cfg_ = std::move(async_tx_checker->storage_phase_cfg_);
    compute_phase_cfg_ = std::move(async_tx_checker->compute_phase_cfg_);
    block_limit_status_ = std::move(async_tx_checker->block_limit_status_);

    // message processing settings
    msg_proc_lt_ = std::move(async_tx_checker->msg_proc_lt_);
    action_phase_cfg_ = std::move(async_tx_checker->action_phase_cfg_);
    
    // deferred messages accounts and total burned data
    account_expected_defer_all_messages_ = std::move(async_tx_checker->account_expected_defer_all_messages_);
    total_burned_ = std::move(async_tx_checker->total_burned_);
...
bool ContestValidateQuery::try_validate() {
  ...
  try {
    ...
    if (!start_async_transactions_check()) {
      return reject_query("cannot run async transaction checker");
    }
    ...
    if (!wait_for_async_transactions_check()){
      return reject_query("invalid collection of account transactions in ShardAccountBlocks");
    }
