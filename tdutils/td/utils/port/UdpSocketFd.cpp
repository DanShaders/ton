...
class UdpSocketFdImpl : private Iocp::Callback {
 public:
  ...
  uint64 get_rx_queue_drops() const {
    return 0;  // SO_RXQ_OVFL is not wired on Windows
  }
...
class UdpSocketFdImpl {
 ...
 private:
  ...
  // Cumulative kernel receive-queue drops (SO_RXQ_OVFL). The per-datagram cmsg carries a monotonic
  // counter; we advance our total only when it grows, so a spurious 0 (e.g. an error-queue read with
  // no cmsg) or a u32 wrap can never inflate the count.
...
#if defined(SO_RXQ_OVFL)
  // Ask the kernel to report receive-queue overflow drops as a per-datagram control message; consumed
  // in from_native() and surfaced via get_rx_queue_drops(). Best-effort: ignored if unsupported.
  {
    int on = 1;
