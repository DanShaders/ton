...
td::Status QuicServer::send_stateless_datagram(td::Slice packet_kind, const td::IPAddress &remote_address,
  ...
  if (!is_sent) {
    ...
    egress.dropped.at(metrics::Reason::limited).inc();
...
td::actor::Task<ServerStats> QuicServer::collect() {
  ...
  if (rx_drops > rx_queue_drops_reflected_) {
    udp_wire_.record_dropped(metrics::Direction::in, metrics::Reason::limited, rx_drops - rx_queue_drops_reflected_);
...
void QuicServer::log_stats(std::string reason) {
  ...
  LOG(INFO) << "quic stats (" << reason << "): udp ingress{syscalls=" << ingress.syscalls.value()
            << " packets=" << ingress.packets.value() << " bytes=" << ingress.bytes.value()
            << "} egress{syscalls=" << egress.syscalls.value() << " packets=" << egress.packets.value()
            << " bytes=" << egress.bytes.value() << "}";
  ...
  while (bytes_budget > 0) {
    ...
    for (size_t i = 0; i < cnt; i++) {
      ...
      if (ingress_errors_[i].is_error()) {
        ...
        udp_wire_.record_dropped(metrics::Direction::in, metrics::Reason::invalid);
      ...
      udp_wire_.dir.at(metrics::Direction::in).bytes.inc(ingress_msg_[i].storage.size());
      const size_t segment_size = ingress_messages_[i].gso_size;

      auto handle_packet = [&](UdpMessageBuffer &packet) {
        udp_wire_.dir.at(metrics::Direction::in).packets.inc();
...
bool QuicServer::flush_pending() {
  ...
  for (size_t i = pending_batch_sent_; i < pending_batch_sent_ + sent_count; i++) {
    egress.bytes.inc(egress_messages_[i].data.size());
    size_t gso_size = egress_messages_[i].gso_size;
    if (gso_size > 0 && egress_messages_[i].data.size() > gso_size) {
      egress.packets.inc((egress_messages_[i].data.size() + gso_size - 1) / gso_size);
    } else {
      egress.packets.inc();
    }
  }
