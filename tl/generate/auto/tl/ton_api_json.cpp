...
Result<int32> tl_constructor_from_string(ton_api::collatorNode_Candidate *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    {"collatorNode.candidate", 540932103},
    {"collatorNode.compressedCandidate", -1174932661}
  };
  auto it = m.find(str);
  if (it == m.end()) {
    return Status::Error(PSLICE() << "Unknown class \"" << str << "\"");
...
Result<int32> tl_constructor_from_string(ton_api::db_filedb_Key *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    ...
    {"db.filedb.key.candidateRef", -431144258},
...
Result<int32> tl_constructor_from_string(ton_api::db_state_Key *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    ...
    {"db.state.key.persistentStateDescriptionShards", -1852061816},
    {"db.state.key.persistentStateDescriptionsList", 1511487946}
  };
  auto it = m.find(str);
  if (it == m.end()) {
    return Status::Error(PSLICE() << "Unknown class \"" << str << "\"");
...
Result<int32> tl_constructor_from_string(ton_api::liteserver_descV2_Slice *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    {"liteserver.descV2.sliceSimple", -2000071413},
    {"liteserver.descV2.sliceTimed", -1018221070}
  };
  auto it = m.find(str);
  if (it == m.end()) {
    return Status::Error(PSLICE() << "Unknown class \"" << str << "\"");
...
Result<int32> tl_constructor_from_string(ton_api::overlay_MemberCertificate *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    {"overlay.memberCertificate", -1040151463},
    {"overlay.emptyMemberCertificate", -1071365662}
  };
  auto it = m.find(str);
  if (it == m.end()) {
    return Status::Error(PSLICE() << "Unknown class \"" << str << "\"");
...
Result<int32> tl_constructor_from_string(ton_api::overlay_Message *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    {"overlay.message", 1965368352},
    {"overlay.messageWithExtra", -1573772483}
  };
  auto it = m.find(str);
  if (it == m.end()) {
    return Status::Error(PSLICE() << "Unknown class \"" << str << "\"");
...
Result<int32> tl_constructor_from_string(ton_api::overlay_db_Nodes *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    {"overlay.db.nodesV2", 917053760},
...
Result<int32> tl_constructor_from_string(ton_api::overlay_node_ToSign *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    {"overlay.node.toSign", 64530657},
    {"overlay.node.toSignEx", -2072352791}
  };
  auto it = m.find(str);
  if (it == m.end()) {
    return Status::Error(PSLICE() << "Unknown class \"" << str << "\"");
...
Result<int32> tl_constructor_from_string(ton_api::tonNode_Broadcast *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    ...
    {"tonNode.blockBroadcastCompressed", 167581507},
    ...
    {"tonNode.newBlockCandidateBroadcast", -1074983488},
    {"tonNode.newBlockCandidateBroadcastCompressed", 215509140},
    {"tonNode.outMsgQueueProofBroadcast", 912191409}
  };
  auto it = m.find(str);
  if (it == m.end()) {
    return Status::Error(PSLICE() << "Unknown class \"" << str << "\"");
...
Result<int32> tl_constructor_from_string(ton_api::tonNode_DataFull *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    ...
    {"tonNode.dataFullCompressed", 1178414245},
...
Result<int32> tl_constructor_from_string(ton_api::tonNode_OutMsgQueueProof *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    {"tonNode.outMsgQueueProof", 1543642066},
    {"tonNode.outMsgQueueProofEmpty", -1600516382}
  };
  auto it = m.find(str);
  if (it == m.end()) {
    return Status::Error(PSLICE() << "Unknown class \"" << str << "\"");
...
Result<int32> tl_constructor_from_string(ton_api::validatorSession_Candidate *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    {"validatorSession.candidate", 2100525125},
    {"validatorSession.compressedCandidate", 1108526967}
  };
  auto it = m.find(str);
  if (it == m.end()) {
    return Status::Error(PSLICE() << "Unknown class \"" << str << "\"");
...
Result<int32> tl_constructor_from_string(ton_api::Object *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    ...
    {"hashable.cntSortedVector", 2073445977},
    {"hashable.validatorSession", 1746035669},
    {"tonNode.importedMsgQueueLimits", -137501941},
    ...
    {"adnl.stats.ipPackets", -1566159974},
    {"adnl.stats.localId", 485643774},
    {"adnl.stats.localIdPackets", -181192842},
    {"adnl.stats.packets", -1660195509},
    {"adnl.stats.peerPair", -1450386620},
    ...
    {"collatorNode.candidate", 540932103},
    {"collatorNode.compressedCandidate", -1174932661},
    {"collatorNode.error", -1896906830},
    {"collatorNode.pong", 1539245345},
    {"config.global", -261690960},
    {"config.local", 2023657820},
    {"contest.test", 935309609},
    ...
    {"db.state.key.persistentStateDescriptionShards", -1852061816},
    {"db.state.key.persistentStateDescriptionsList", 1511487946},
    {"db.state.persistentStateDescriptionHeader", -1325128876},
    {"db.state.persistentStateDescriptionShards", -433023920},
    {"db.state.persistentStateDescriptionsList", -1485326599},
    ...
    {"engine.validator.collationManagerStats", 402697037},
    {"engine.validator.collatorNodeWhitelist", -484118479},
    {"engine.validator.collatorOptions", 71182465},
    {"engine.validator.collatorsList", -350754052},
    {"engine.validator.config", -1647573204},
    {"engine.validator.controlQueryError", 1999018527},
    {"engine.validator.customOverlay", -1536918323},
    {"engine.validator.customOverlayNode", 692575999},
    {"engine.validator.customOverlaysConfig", 1081752345},
    ...
    {"engine.validator.exportedPrivateKeys", -1189054063},
    {"engine.validator.extraConfig", 847549418},
    {"engine.validator.fastSyncMemberCertificate", -722821601},
    {"engine.validator.fastSyncOverlayClient", 1395058805},
    {"engine.validator.fastSyncOverlayStats", -14698336},
    ...
    {"engine.validator.overlayStats", 1874130251},
    {"engine.validator.overlayStatsNode", -775469870},
    {"engine.validator.overlayStatsTraffic", 1494746854},
    ...
    {"engine.validator.shardOutQueueSize", 2098409467},
    {"engine.validator.shardOverlayStats", 1131748080},
    {"engine.validator.signature", -76791000},
    {"engine.validator.stats", 1565119343},
    {"engine.validator.success", -1276860789},
    {"engine.validator.textStats", -1176803271},
    {"engine.validator.time", -547380738},
    {"engine.validator.collationManagerStats.collator", 388106561},
    {"engine.validator.collationManagerStats.localId", -1807470882},
    {"engine.validator.collationManagerStats.shard", -123361695},
    {"engine.validator.collatorsList.collator", -786459762},
    {"engine.validator.collatorsList.shard", 1380958561},
    {"engine.validator.shardOverlayStats.neighbour", -1723992174},
    {"fec.raptorQ", -1953257504},
    {"fec.roundRobin", 854927588},
    {"fec.online", 19359244},
    ...
    {"liteclient.config.global", -393642067},
    {"liteserver.desc", -1001806732},
    {"liteserver.descV2", 1259281547},
    {"liteserver.config.local", 1182002063},
    {"liteserver.config.random.local", 2093565243},
    {"liteserver.descV2.shardInfo", 36288420},
    {"liteserver.descV2.sliceSimple", -2000071413},
    {"liteserver.descV2.sliceTimed", -1018221070},
    {"overlay.fec.received", -715385620},
    {"overlay.fec.completed", 165112084},
    ...
    {"overlay.memberCertificate", -1040151463},
    {"overlay.emptyMemberCertificate", -1071365662},
    {"overlay.memberCertificateId", 747101234},
    {"overlay.message", 1965368352},
    {"overlay.messageWithExtra", -1573772483},
    {"overlay.messageExtra", 785967269},
    {"overlay.node", -1200911741},
    {"overlay.nodeV2", -1113870937},
    {"overlay.nodes", -460904178},
    {"overlay.nodesV2", 449963576},
    ...
    {"overlay.node.toSign", 64530657},
    {"overlay.node.toSignEx", -2072352791},
    {"proxyLiteserver.config", 808543630},
    {"rldp.message", 2098973982},
    {"rldp.query", -1971761815},
    {"rldp.answer", -1543742461},
    {"rldp.messagePart", 408691404},
    ...
    {"tonNode.customOverlayId", 968105638},
    {"tonNode.data", 1443505284},
    {"tonNode.dataFull", -1101488237},
    {"tonNode.dataFullCompressed", 1178414245},
    {"tonNode.dataFullEmpty", 1466861002},
    {"tonNode.externalMessage", -596270583},
    {"tonNode.fastSyncOverlayId", 452003156},
    {"tonNode.forgetPeer", -1423772543},
    {"tonNode.ihrMessage", 1161085703},
    {"tonNode.keyBlocks", 124144985},
    {"tonNode.newFastSyncMemberCertificate", 1939862959},
    ...
    {"tonNode.shardId", 687474807},
    {"tonNode.shardPublicOverlayId", 1302254377},
    {"tonNode.success", -1063902129},
    {"tonNode.zeroStateIdExt", 494024110},
    {"tonNode.blockBroadcastCompressed.data", -24046666},
    {"validator.group", -120029535},
    {"validator.groupEx", 479350270},
    {"validator.groupNew", -1740398259},
    {"validator.telemetry", 1560508273},
    ...
    {"validatorSession.message.precommit", -1470843566},
    {"validatorSession.message.empty", 1243619241},
    {"validatorStats.collatedBlock", -316530976},
    {"validatorSession.candidate.id", -1126743751},
    {"validatorStats.blockLimitsStatus", -1603837974},
    {"validatorStats.collatorNodeResponse", 474244860},
    {"validatorStats.endValidatorGroup", -1593008322},
    {"validatorStats.extMsgsStats", 1803066447},
    {"validatorStats.newValidatorGroup", -1914588615},
    {"validatorStats.stats", 1157091964},
    {"validatorStats.validatedBlock", 1396250433},
    {"validatorStats.endValidatorGroup.node", 2048931237},
    {"validatorStats.newValidatorGroup.node", -1138988284},
    {"validatorStats.stats.producer", -1121712627},
    {"validatorStats.stats.round", 620582254}
  };
  auto it = m.find(str);
  if (it == m.end()) {
    return Status::Error(PSLICE() << "Unknown class \"" << str << "\"");
...
Result<int32> tl_constructor_from_string(ton_api::Function *object, const std::string &str) {
  static const std::unordered_map<Slice, int32, SliceHash> m = {
    ...
    {"collatorNode.generateBlock", -464047564},
    {"collatorNode.ping", 2112845767},
    ...
    {"engine.validator.clearCollatorsList", -957042098},
    {"engine.validator.collatorNodeSetWhitelistEnabled", -136165035},
    {"engine.validator.collatorNodeSetWhitelistedValidator", 365758025},
    ...
    {"engine.validator.delCollator", 293914972},
    {"engine.validator.delCustomOverlay", 2034844268},
    {"engine.validator.delDhtId", -2063770818},
    {"engine.validator.delFastSyncClient", -301475743},
    {"engine.validator.delListeningPort", 828094543},
    {"engine.validator.delProxy", 1970850941},
    {"engine.validator.delShard", 1328182971},
    ...
    {"engine.validator.exportAllPrivateKeys", -849980430},
    ...
    {"engine.validator.getActorTextStats", 1755376848},
    {"engine.validator.getAdnlStats", -1818239261},
    {"engine.validator.getCollationManagerStats", -430816772},
    {"engine.validator.getCollatorOptionsJson", 1279450873},
    ...
    {"engine.validator.importFastSyncMemberCertificate", 365076401},
    {"engine.validator.importPrivateKey", 360741575},
    {"engine.validator.importShardOverlayCertificate", 448989784},
    {"engine.validator.setCollatorOptionsJson", 1503354240},
    {"engine.validator.setCollatorsList", 1500768396},
    {"engine.validator.setExtMessagesBroadcastDisabled", -1970206246},
    {"engine.validator.setStateSerializerEnabled", 2057046485},
    {"engine.validator.setVerbosity", -1316856190},
    {"engine.validator.showCollatorNodeWhitelist", -2141136903},
    {"engine.validator.showCollatorsList", 801133077},
    {"engine.validator.showCustomOverlays", -1186851525},
    {"engine.validator.sign", 451549736},
    {"engine.validator.signOverlayMemberCertificate", 1098167522},
    ...
    {"overlay.getRandomPeersV2", 362819600},
    {"overlay.query", -855800765},
    {"overlay.queryWithExtra", -1795177495},
    ...
    {"tonNode.getOutMsgQueueProof", -2075371231},
    {"tonNode.getPrevBlocksDescription", 1550675145},
    {"tonNode.getShardArchiveInfo", 318454616},
    ...
    {"tonNode.requestFastSyncOverlayMemberCertificate", 1429729957},
...
Status from_json(ton_api::tonNode_importedMsgQueueLimits &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "max_bytes", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.max_bytes_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "max_msgs", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.max_msgs_, std::move(value)));
    }
  }
  return Status::OK();
}
...
Status from_json(ton_api::adnl_stats &to, JsonObject &from) {
  {
...
Status from_json(ton_api::adnl_stats_ipPackets &to, JsonObject &from) {
  {
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "packets", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.packets_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::adnl_stats_localId &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "short_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.short_id_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "current_decrypt", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.current_decrypt_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "packets_recent", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.packets_recent_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "packets_total", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.packets_total_, std::move(value)));
    }
  }
  {
...
Status from_json(ton_api::adnl_stats_localIdPackets &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "ts_start", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.ts_start_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "ts_end", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.ts_end_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "decrypted_packets", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.decrypted_packets_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "dropped_packets", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.dropped_packets_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::adnl_stats_packets &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "ts_start", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.ts_start_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "in_packets", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.in_packets_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "in_bytes", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.in_bytes_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "in_packets_channel", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.in_packets_channel_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "in_bytes_channel", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.in_bytes_channel_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "out_packets", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.out_packets_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "out_bytes", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.out_bytes_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "out_packets_channel", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.out_packets_channel_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "out_bytes_channel", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.out_bytes_channel_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "out_expired_messages", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.out_expired_messages_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "out_expired_bytes", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.out_expired_bytes_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::adnl_stats_peerPair &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "local_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.local_id_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "last_out_packet_ts", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.last_out_packet_ts_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "last_in_packet_ts", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.last_in_packet_ts_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "connection_ready", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.connection_ready_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "channel_status", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.channel_status_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "try_reinit_at", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.try_reinit_at_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "out_queue_messages", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.out_queue_messages_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "out_queue_bytes", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.out_queue_bytes_, std::move(value)));
    }
  }
  return Status::OK();
}
...
Status from_json(ton_api::collatorNode_candidate &to, JsonObject &from) {
  {
...
Status from_json(ton_api::collatorNode_compressedCandidate &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "flags", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.flags_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "decompressed_size", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.decompressed_size_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "data", JsonValue::Type::Null, true));
...
Status from_json(ton_api::collatorNode_error &to, JsonObject &from) {
  {
...
Status from_json(ton_api::collatorNode_pong &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "flags", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.flags_, std::move(value)));
...
Status from_json(ton_api::contest_test &to, JsonObject &from) {
  {
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "block_data", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json_bytes(to.block_data_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "collated_data", JsonValue::Type::Null, true));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "valid", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.valid_, std::move(value)));
    }
  }
  return Status::OK();
}
...
Status from_json(ton_api::db_filedb_key_candidateRef &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.id_, std::move(value)));
...
Status from_json(ton_api::db_state_key_persistentStateDescriptionShards &to, JsonObject &from) {
  {
...
Status from_json(ton_api::db_state_key_persistentStateDescriptionsList &to, JsonObject &from) {
  return Status::OK();
}
Status from_json(ton_api::db_state_persistentStateDescriptionHeader &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "masterchain_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.masterchain_id_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "start_time", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.start_time_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "end_time", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.end_time_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::db_state_persistentStateDescriptionShards &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "shard_blocks", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.shard_blocks_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::db_state_persistentStateDescriptionsList &to, JsonObject &from) {
  {
...
Status from_json(ton_api::engine_collator &to, JsonObject &from) {
  {
...
Status from_json(ton_api::engine_validator_collationManagerStats &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "local_ids", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.local_ids_, std::move(value)));
...
Status from_json(ton_api::engine_validator_collatorNodeWhitelist &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "enabled", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.enabled_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "adnl_ids", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.adnl_ids_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::engine_validator_collatorOptions &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "deferring_enabled", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.deferring_enabled_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "defer_messages_after", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.defer_messages_after_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "defer_out_queue_size_limit", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.defer_out_queue_size_limit_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "dispatch_phase_2_max_total", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.dispatch_phase_2_max_total_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "dispatch_phase_3_max_total", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.dispatch_phase_3_max_total_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "dispatch_phase_2_max_per_initiator", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.dispatch_phase_2_max_per_initiator_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "dispatch_phase_3_max_per_initiator", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.dispatch_phase_3_max_per_initiator_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "whitelist", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.whitelist_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "prioritylist", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.prioritylist_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "force_full_collated_data", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.force_full_collated_data_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "ignore_collated_data_limits", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.ignore_collated_data_limits_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::engine_validator_collatorsList &to, JsonObject &from) {
  {
...
Status from_json(ton_api::engine_validator_config &to, JsonObject &from) {
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "collators", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.collators_, std::move(value)));
    }
  }
  {
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "extraconfig", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.extraconfig_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "liteservers", JsonValue::Type::Null, true));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "shards_to_monitor", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.shards_to_monitor_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "gc", JsonValue::Type::Null, true));
...
Status from_json(ton_api::engine_validator_customOverlay &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "name", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "sender_shards", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.sender_shards_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::engine_validator_customOverlayNode &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "adnl_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.adnl_id_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "msg_sender", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.msg_sender_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "msg_sender_priority", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.msg_sender_priority_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "block_sender", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.block_sender_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::engine_validator_customOverlaysConfig &to, JsonObject &from) {
  {
...
Status from_json(ton_api::engine_validator_exportedPrivateKeys &to, JsonObject &from) {
  {
...
Status from_json(ton_api::engine_validator_extraConfig &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "state_serializer_enabled", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.state_serializer_enabled_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "fast_sync_member_certificates", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.fast_sync_member_certificates_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "collator_node_whitelist", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.collator_node_whitelist_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "fast_sync_overlay_clients", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.fast_sync_overlay_clients_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::engine_validator_fastSyncMemberCertificate &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "adnl_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.adnl_id_, std::move(value)));
...
Status from_json(ton_api::engine_validator_fastSyncOverlayClient &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "adnl_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.adnl_id_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "slot", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.slot_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::engine_validator_fastSyncOverlayStats &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "shard", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.shard_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "validators_adnl", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.validators_adnl_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "root_public_keys", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.root_public_keys_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "member_certificate", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.member_certificate_, std::move(value)));
    }
  }
  return Status::OK();
}
...
Status from_json(ton_api::engine_validator_overlayStats &to, JsonObject &from) {
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "total_traffic", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.total_traffic_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "total_traffic_responses", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.total_traffic_responses_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "extra", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.extra_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::engine_validator_overlayStatsNode &to, JsonObject &from) {
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "is_neighbour", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.is_neighbour_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "is_alive", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.is_alive_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "node_flags", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.node_flags_, std::move(value)));
    }
  }
  {
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "traffic", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.traffic_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "traffic_responses", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.traffic_responses_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::engine_validator_overlayStatsTraffic &to, JsonObject &from) {
  {
...
Status from_json(ton_api::engine_validator_shardOverlayStats &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "shard", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.shard_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "neighbours", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.neighbours_, std::move(value)));
    }
  }
  return Status::OK();
}
...
Status from_json(ton_api::engine_validator_textStats &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "data", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.data_, std::move(value)));
...
Status from_json(ton_api::engine_validator_collationManagerStats_collator &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "adnl_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.adnl_id_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "alive", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.alive_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "ping_in", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.ping_in_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "last_ping_ago", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.last_ping_ago_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "last_ping_status", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.last_ping_status_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::engine_validator_collationManagerStats_localId &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "adnl_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.adnl_id_, std::move(value)));
...
Status from_json(ton_api::engine_validator_collationManagerStats_shard &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "shard_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.shard_id_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "self_collate", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.self_collate_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "select_mode", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.select_mode_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "active", JsonValue::Type::Null, true));
...
Status from_json(ton_api::engine_validator_collatorsList_collator &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "adnl_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.adnl_id_, std::move(value)));
...
Status from_json(ton_api::engine_validator_collatorsList_shard &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "shard_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.shard_id_, std::move(value)));
...
Status from_json(ton_api::engine_validator_shardOverlayStats_neighbour &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.id_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "verison_major", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.verison_major_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "version_minor", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.version_minor_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "flags", JsonValue::Type::Null, true));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "roundtrip", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.roundtrip_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "unreliability", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.unreliability_, std::move(value)));
    }
  }
  return Status::OK();
}
...
Status from_json(ton_api::liteclient_config_global &to, JsonObject &from) {
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "liteservers_v2", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.liteservers_v2_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "validator", JsonValue::Type::Null, true));
...
Status from_json(ton_api::liteserver_descV2 &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.id_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "slices", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.slices_, std::move(value)));
    }
  }
  return Status::OK();
}
...
Status from_json(ton_api::liteserver_descV2_shardInfo &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "shard_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.shard_id_, std::move(value)));
...
Status from_json(ton_api::liteserver_descV2_sliceSimple &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "shards", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.shards_, std::move(value)));
...
Status from_json(ton_api::liteserver_descV2_sliceTimed &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "shards_from", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.shards_from_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "shards_to", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.shards_to_, std::move(value)));
    }
  }
  return Status::OK();
}
...
Status from_json(ton_api::overlay_memberCertificate &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "issued_by", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.issued_by_, std::move(value)));
...
Status from_json(ton_api::overlay_emptyMemberCertificate &to, JsonObject &from) {
  return Status::OK();
}
Status from_json(ton_api::overlay_memberCertificateId &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "node", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.node_, std::move(value)));
...
Status from_json(ton_api::overlay_messageWithExtra &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "overlay", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.overlay_, std::move(value)));
...
Status from_json(ton_api::overlay_messageExtra &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "flags", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.flags_, std::move(value)));
...
Status from_json(ton_api::overlay_nodeV2 &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.id_, std::move(value)));
...
Status from_json(ton_api::overlay_nodesV2 &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "nodes", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.nodes_, std::move(value)));
...
Status from_json(ton_api::overlay_db_nodesV2 &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "nodes", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.nodes_, std::move(value)));
...
Status from_json(ton_api::overlay_node_toSignEx &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.id_, std::move(value)));
...
Status from_json(ton_api::proxyLiteserver_config &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "port", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.port_, std::move(value)));
...
Status from_json(ton_api::tonNode_blockBroadcastCompressed &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.id_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "compressed", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json_bytes(to.compressed_, std::move(value)));
    }
  }
  return Status::OK();
}
...
Status from_json(ton_api::tonNode_newBlockCandidateBroadcast &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.id_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "collator_signature", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.collator_signature_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "data", JsonValue::Type::Null, true));
...
Status from_json(ton_api::tonNode_newBlockCandidateBroadcastCompressed &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.id_, std::move(value)));
...
Status from_json(ton_api::tonNode_outMsgQueueProofBroadcast &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "dst_shard", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.dst_shard_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "block", JsonValue::Type::Null, true));
  ...
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "proof", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.proof_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::tonNode_capabilities &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "version_major", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.version_major_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "version_minor", JsonValue::Type::Null, true));
...
Status from_json(ton_api::tonNode_customOverlayId &to, JsonObject &from) {
  {
...
Status from_json(ton_api::tonNode_dataFullCompressed &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.id_, std::move(value)));
...
Status from_json(ton_api::tonNode_fastSyncOverlayId &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "zero_state_file_hash", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.zero_state_file_hash_, std::move(value)));
...
Status from_json(ton_api::tonNode_forgetPeer &to, JsonObject &from) {
  return Status::OK();
}
...
Status from_json(ton_api::tonNode_newFastSyncMemberCertificate &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "adnl_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.adnl_id_, std::move(value)));
...
Status from_json(ton_api::tonNode_outMsgQueueProof &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "queue_proofs", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json_bytes(to.queue_proofs_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "block_state_proofs", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json_bytes(to.block_state_proofs_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "msg_counts", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.msg_counts_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::tonNode_outMsgQueueProofEmpty &to, JsonObject &from) {
  return Status::OK();
}
...
Status from_json(ton_api::tonNode_shardId &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "workchain", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.workchain_, std::move(value)));
...
Status from_json(ton_api::tonNode_blockBroadcastCompressed_data &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "signatures", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.signatures_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "proof_data", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json_bytes(to.proof_data_, std::move(value)));
    }
  }
  return Status::OK();
}
...
Status from_json(ton_api::validator_telemetry &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "flags", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.flags_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "node_version", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.node_version_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "os_version", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.os_version_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "node_started_at", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.node_started_at_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "ram_size", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.ram_size_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "cpu_cores", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.cpu_cores_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "node_threads", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.node_threads_, std::move(value)));
    }
  }
  return Status::OK();
}
...
Status from_json(ton_api::validatorSession_compressedCandidate &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "flags", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.flags_, std::move(value)));
...
Status from_json(ton_api::validatorStats_collatedBlock &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "block_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.block_id_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "collated_data_hash", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.collated_data_hash_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "cc_seqno", JsonValue::Type::Null, true));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "collated_at", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.collated_at_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "bytes", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.bytes_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "collated_data_bytes", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.collated_data_bytes_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "attempt", JsonValue::Type::Null, true));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "is_validator", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.is_validator_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "total_time", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.total_time_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "work_time", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.work_time_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "cpu_work_time", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.cpu_work_time_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "time_stats", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.time_stats_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "block_limits", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.block_limits_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "ext_msgs_stats", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.ext_msgs_stats_, std::move(value)));
    }
  }
  return Status::OK();
}
...
Status from_json(ton_api::validatorStats_blockLimitsStatus &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "bytes", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.bytes_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "gas", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.gas_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "lt_delta", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.lt_delta_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "collated_data_bytes", JsonValue::Type::Null, true));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "cat_bytes", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.cat_bytes_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "cat_gas", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.cat_gas_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "cat_lt_delta", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.cat_lt_delta_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "cat_collated_data_bytes", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.cat_collated_data_bytes_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "limits_log", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.limits_log_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::validatorStats_collatorNodeResponse &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "self", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.self_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "validator_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.validator_id_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "timestamp", JsonValue::Type::Null, true));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "original_block_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.original_block_id_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "collated_data_hash", JsonValue::Type::Null, true));
...
Status from_json(ton_api::validatorStats_endValidatorGroup &to, JsonObject &from) {
  {
...
Status from_json(ton_api::validatorStats_extMsgsStats &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "ext_msgs_total", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.ext_msgs_total_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "ext_msgs_filtered", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.ext_msgs_filtered_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "ext_msgs_accepted", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.ext_msgs_accepted_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "ext_msgs_rejected", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.ext_msgs_rejected_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::validatorStats_newValidatorGroup &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "session_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.session_id_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "started_at", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.started_at_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "self_idx", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.self_idx_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "self", JsonValue::Type::Null, true));
...
Status from_json(ton_api::validatorStats_stats &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "flags", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.flags_, std::move(value)));
  ...
  }
  {
    ...
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.success_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "timestamp", JsonValue::Type::Null, true));
...
Status from_json(ton_api::validatorStats_validatedBlock &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "block_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.block_id_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "validated_at", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.validated_at_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "self", JsonValue::Type::Null, true));
  ...
  }
  {
    ...
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.comment_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "bytes", JsonValue::Type::Null, true));
...
Status from_json(ton_api::validatorStats_endValidatorGroup_node &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.id_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "catchain_blocks", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.catchain_blocks_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::validatorStats_newValidatorGroup_node &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.id_, std::move(value)));
...
Status from_json(ton_api::validatorStats_stats_producer &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "flags", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.flags_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "candidate_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.candidate_id_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "block_id", JsonValue::Type::Null, true));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "is_accepted", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.is_accepted_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "is_ours", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.is_ours_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "got_block_at", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.got_block_at_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "got_submit_at", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.got_submit_at_, std::move(value)));
    }
  }
  {
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "collation_time", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.collation_time_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "collated_at", JsonValue::Type::Null, true));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "collation_cached", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.collation_cached_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "self_collated", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.self_collated_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "collator_node_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.collator_node_id_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "validation_time", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.validation_time_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "validated_at", JsonValue::Type::Null, true));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "validation_cached", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.validation_cached_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "approved_weight", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.approved_weight_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "approved_33pct_at", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.approved_33pct_at_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "approved_66pct_at", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.approved_66pct_at_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "approvers", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.approvers_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "signed_weight", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.signed_weight_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "signed_33pct_at", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.signed_33pct_at_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "signed_66pct_at", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.signed_66pct_at_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "signers", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.signers_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "serialize_time", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.serialize_time_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "deserialize_time", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.deserialize_time_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "serialized_size", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.serialized_size_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::validatorStats_stats_round &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "started_at", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.started_at_, std::move(value)));
...
Status from_json(ton_api::collatorNode_generateBlock &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "shard", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.shard_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "first_block_round", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.first_block_round_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "priority", JsonValue::Type::Null, true));
...
Status from_json(ton_api::collatorNode_ping &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "flags", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.flags_, std::move(value)));
...
Status from_json(ton_api::engine_validator_addCollator &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "adnl_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.adnl_id_, std::move(value)));
...
Status from_json(ton_api::engine_validator_addCustomOverlay &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "overlay", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.overlay_, std::move(value)));
...
Status from_json(ton_api::engine_validator_addFastSyncClient &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "adnl_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.adnl_id_, std::move(value)));
...
Status from_json(ton_api::engine_validator_addShard &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "shard", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.shard_, std::move(value)));
...
Status from_json(ton_api::engine_validator_clearCollatorsList &to, JsonObject &from) {
  return Status::OK();
}
Status from_json(ton_api::engine_validator_collatorNodeSetWhitelistEnabled &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "enabled", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.enabled_, std::move(value)));
...
Status from_json(ton_api::engine_validator_collatorNodeSetWhitelistedValidator &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "adnl_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.adnl_id_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "add", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.add_, std::move(value)));
    }
  }
  return Status::OK();
}
...
Status from_json(ton_api::engine_validator_delCollator &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "adnl_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.adnl_id_, std::move(value)));
...
Status from_json(ton_api::engine_validator_delCustomOverlay &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "name", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.name_, std::move(value)));
...
Status from_json(ton_api::engine_validator_delFastSyncClient &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "adnl_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.adnl_id_, std::move(value)));
...
Status from_json(ton_api::engine_validator_delShard &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "shard", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.shard_, std::move(value)));
...
Status from_json(ton_api::engine_validator_exportAllPrivateKeys &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "encryption_key", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.encryption_key_, std::move(value)));
    }
  }
  return Status::OK();
}
...
Status from_json(ton_api::engine_validator_getActorTextStats &to, JsonObject &from) {
  return Status::OK();
}
Status from_json(ton_api::engine_validator_getAdnlStats &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "all", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.all_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::engine_validator_getCollationManagerStats &to, JsonObject &from) {
  return Status::OK();
}
Status from_json(ton_api::engine_validator_getCollatorOptionsJson &to, JsonObject &from) {
  return Status::OK();
}
...
Status from_json(ton_api::engine_validator_importFastSyncMemberCertificate &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "adnl_id", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.adnl_id_, std::move(value)));
...
Status from_json(ton_api::engine_validator_setCollatorOptionsJson &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "json", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.json_, std::move(value)));
    }
  }
  return Status::OK();
}
Status from_json(ton_api::engine_validator_setCollatorsList &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "list", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.list_, std::move(value)));
...
Status from_json(ton_api::engine_validator_setStateSerializerEnabled &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "enabled", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.enabled_, std::move(value)));
...
Status from_json(ton_api::engine_validator_showCollatorNodeWhitelist &to, JsonObject &from) {
  return Status::OK();
}
Status from_json(ton_api::engine_validator_showCollatorsList &to, JsonObject &from) {
  return Status::OK();
}
Status from_json(ton_api::engine_validator_showCustomOverlays &to, JsonObject &from) {
  return Status::OK();
}
...
Status from_json(ton_api::engine_validator_signOverlayMemberCertificate &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "sign_by", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.sign_by_, std::move(value)));
    }
  }
  {
    TRY_RESULT(value, get_json_object_field(from, "adnl_id", JsonValue::Type::Null, true));
...
Status from_json(ton_api::overlay_getRandomPeersV2 &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "peers", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.peers_, std::move(value)));
...
Status from_json(ton_api::overlay_queryWithExtra &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "overlay", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.overlay_, std::move(value)));
...
Status from_json(ton_api::tonNode_getOutMsgQueueProof &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "dst_shard", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.dst_shard_, std::move(value)));
...
Status from_json(ton_api::tonNode_getShardArchiveInfo &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "masterchain_seqno", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.masterchain_seqno_, std::move(value)));
  ...
  {
    TRY_RESULT(value, get_json_object_field(from, "shard_prefix", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.shard_prefix_, std::move(value)));
    }
  }
  return Status::OK();
}
...
Status from_json(ton_api::tonNode_requestFastSyncOverlayMemberCertificate &to, JsonObject &from) {
  {
    TRY_RESULT(value, get_json_object_field(from, "sign_by", JsonValue::Type::Null, true));
    if (value.type() != JsonValue::Type::Null) {
      TRY_STATUS(from_json(to.sign_by_, std::move(value)));
...
void to_json(JsonValueScope &jv, const ton_api::tonNode_importedMsgQueueLimits &object) {
  auto jo = jv.enter_object();
  jo("@type", "tonNode.importedMsgQueueLimits");
  jo("max_bytes", ToJson(object.max_bytes_));
  jo("max_msgs", ToJson(object.max_msgs_));
}
...
void to_json(JsonValueScope &jv, const ton_api::adnl_stats &object) {
  auto jo = jv.enter_object();
  jo("@type", "adnl.stats");
  jo("timestamp", ToJson(object.timestamp_));
...
void to_json(JsonValueScope &jv, const ton_api::adnl_stats_ipPackets &object) {
  auto jo = jv.enter_object();
  jo("@type", "adnl.stats.ipPackets");
  jo("ip_str", ToJson(object.ip_str_));
  jo("packets", ToJson(JsonInt64{object.packets_}));
}
void to_json(JsonValueScope &jv, const ton_api::adnl_stats_localId &object) {
  auto jo = jv.enter_object();
  jo("@type", "adnl.stats.localId");
  jo("short_id", ToJson(object.short_id_));
  jo("current_decrypt", ToJson(object.current_decrypt_));
  if (object.packets_recent_) {
    jo("packets_recent", ToJson(object.packets_recent_));
  }
  if (object.packets_total_) {
    jo("packets_total", ToJson(object.packets_total_));
  }
  jo("peers", ToJson(object.peers_));
}
void to_json(JsonValueScope &jv, const ton_api::adnl_stats_localIdPackets &object) {
  auto jo = jv.enter_object();
  jo("@type", "adnl.stats.localIdPackets");
  jo("ts_start", ToJson(object.ts_start_));
  jo("ts_end", ToJson(object.ts_end_));
  jo("decrypted_packets", ToJson(object.decrypted_packets_));
  jo("dropped_packets", ToJson(object.dropped_packets_));
}
void to_json(JsonValueScope &jv, const ton_api::adnl_stats_packets &object) {
  auto jo = jv.enter_object();
  jo("@type", "adnl.stats.packets");
  jo("ts_start", ToJson(object.ts_start_));
  jo("ts_end", ToJson(object.ts_end_));
  jo("in_packets", ToJson(JsonInt64{object.in_packets_}));
  jo("in_bytes", ToJson(JsonInt64{object.in_bytes_}));
  jo("in_packets_channel", ToJson(JsonInt64{object.in_packets_channel_}));
  jo("in_bytes_channel", ToJson(JsonInt64{object.in_bytes_channel_}));
  jo("out_packets", ToJson(JsonInt64{object.out_packets_}));
  jo("out_bytes", ToJson(JsonInt64{object.out_bytes_}));
  jo("out_packets_channel", ToJson(JsonInt64{object.out_packets_channel_}));
  jo("out_bytes_channel", ToJson(JsonInt64{object.out_bytes_channel_}));
  jo("out_expired_messages", ToJson(JsonInt64{object.out_expired_messages_}));
  jo("out_expired_bytes", ToJson(JsonInt64{object.out_expired_bytes_}));
}
void to_json(JsonValueScope &jv, const ton_api::adnl_stats_peerPair &object) {
  auto jo = jv.enter_object();
  jo("@type", "adnl.stats.peerPair");
  jo("local_id", ToJson(object.local_id_));
  jo("peer_id", ToJson(object.peer_id_));
  jo("ip_str", ToJson(object.ip_str_));
  if (object.packets_recent_) {
  ...
  jo("last_out_packet_ts", ToJson(object.last_out_packet_ts_));
  jo("last_in_packet_ts", ToJson(object.last_in_packet_ts_));
  jo("connection_ready", ToJson(JsonBool{object.connection_ready_}));
  jo("channel_status", ToJson(object.channel_status_));
  jo("try_reinit_at", ToJson(object.try_reinit_at_));
  jo("out_queue_messages", ToJson(JsonInt64{object.out_queue_messages_}));
  jo("out_queue_bytes", ToJson(JsonInt64{object.out_queue_bytes_}));
}
...
void to_json(JsonValueScope &jv, const ton_api::collatorNode_Candidate &object) {
  ton_api::downcast_call(const_cast<ton_api::collatorNode_Candidate &>(object), [&jv](const auto &object) { to_json(jv, object); });
}
void to_json(JsonValueScope &jv, const ton_api::collatorNode_candidate &object) {
  auto jo = jv.enter_object();
  jo("@type", "collatorNode.candidate");
  if (object.source_) {
...
void to_json(JsonValueScope &jv, const ton_api::collatorNode_compressedCandidate &object) {
  auto jo = jv.enter_object();
  jo("@type", "collatorNode.compressedCandidate");
  if (object.flags_) {
    jo("flags", ToJson(object.flags_));
  }
  if (object.source_) {
  ...
  jo("decompressed_size", ToJson(object.decompressed_size_));
  jo("data", ToJson(JsonBytes{object.data_}));
}
void to_json(JsonValueScope &jv, const ton_api::collatorNode_error &object) {
  auto jo = jv.enter_object();
  jo("@type", "collatorNode.error");
  jo("code", ToJson(object.code_));
  jo("message", ToJson(object.message_));
}
void to_json(JsonValueScope &jv, const ton_api::collatorNode_pong &object) {
  auto jo = jv.enter_object();
  jo("@type", "collatorNode.pong");
  if (object.flags_) {
    jo("flags", ToJson(object.flags_));
  }
}
...
void to_json(JsonValueScope &jv, const ton_api::contest_test &object) {
  auto jo = jv.enter_object();
  jo("@type", "contest.test");
  if (object.block_id_) {
    jo("block_id", ToJson(object.block_id_));
  }
  jo("block_data", ToJson(JsonBytes{object.block_data_}));
  jo("collated_data", ToJson(JsonBytes{object.collated_data_}));
  jo("valid", ToJson(JsonBool{object.valid_}));
}
...
void to_json(JsonValueScope &jv, const ton_api::db_block_info &object) {
  ...
  if (object.prev_left_ && (object.flags_ & 2)) {
    jo("prev_left", ToJson(object.prev_left_));
  }
  if (object.prev_right_ && (object.flags_ & 4)) {
    jo("prev_right", ToJson(object.prev_right_));
  }
  if (object.next_left_ && (object.flags_ & 8)) {
    jo("next_left", ToJson(object.next_left_));
  }
  if (object.next_right_ && (object.flags_ & 16)) {
    jo("next_right", ToJson(object.next_right_));
  }
  if ((object.flags_ & 8192)) {
    jo("lt", ToJson(JsonInt64{object.lt_}));
  }
  if ((object.flags_ & 16384)) {
    jo("ts", ToJson(object.ts_));
  }
  if ((object.flags_ & 131072)) {
    jo("state", ToJson(object.state_));
  }
  if ((object.flags_ & 8388608)) {
...
void to_json(JsonValueScope &jv, const ton_api::db_block_archivedInfo &object) {
  ...
  if (object.next_ && (object.flags_ & 1)) {
...
void to_json(JsonValueScope &jv, const ton_api::db_filedb_key_candidateRef &object) {
  auto jo = jv.enter_object();
  jo("@type", "db.filedb.key.candidateRef");
  if (object.id_) {
    jo("id", ToJson(object.id_));
  }
}
...
void to_json(JsonValueScope &jv, const ton_api::db_state_key_persistentStateDescriptionShards &object) {
  auto jo = jv.enter_object();
  jo("@type", "db.state.key.persistentStateDescriptionShards");
  jo("masterchain_seqno", ToJson(object.masterchain_seqno_));
}
void to_json(JsonValueScope &jv, const ton_api::db_state_key_persistentStateDescriptionsList &object) {
  auto jo = jv.enter_object();
  jo("@type", "db.state.key.persistentStateDescriptionsList");
}
void to_json(JsonValueScope &jv, const ton_api::db_state_persistentStateDescriptionHeader &object) {
  auto jo = jv.enter_object();
  jo("@type", "db.state.persistentStateDescriptionHeader");
  if (object.masterchain_id_) {
    jo("masterchain_id", ToJson(object.masterchain_id_));
  }
  jo("start_time", ToJson(object.start_time_));
  jo("end_time", ToJson(object.end_time_));
}
void to_json(JsonValueScope &jv, const ton_api::db_state_persistentStateDescriptionShards &object) {
  auto jo = jv.enter_object();
  jo("@type", "db.state.persistentStateDescriptionShards");
  jo("shard_blocks", ToJson(object.shard_blocks_));
}
void to_json(JsonValueScope &jv, const ton_api::db_state_persistentStateDescriptionsList &object) {
  auto jo = jv.enter_object();
  jo("@type", "db.state.persistentStateDescriptionsList");
...
void to_json(JsonValueScope &jv, const ton_api::engine_collator &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.collator");
  jo("adnl_id", ToJson(object.adnl_id_));
  if (object.shard_) {
    jo("shard", ToJson(object.shard_));
  }
}
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_collationManagerStats &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.collationManagerStats");
  jo("local_ids", ToJson(object.local_ids_));
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_collatorNodeWhitelist &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.collatorNodeWhitelist");
  jo("enabled", ToJson(JsonBool{object.enabled_}));
  jo("adnl_ids", ToJson(object.adnl_ids_));
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_collatorOptions &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.collatorOptions");
  jo("deferring_enabled", ToJson(JsonBool{object.deferring_enabled_}));
  jo("defer_messages_after", ToJson(object.defer_messages_after_));
  jo("defer_out_queue_size_limit", ToJson(JsonInt64{object.defer_out_queue_size_limit_}));
  jo("dispatch_phase_2_max_total", ToJson(object.dispatch_phase_2_max_total_));
  jo("dispatch_phase_3_max_total", ToJson(object.dispatch_phase_3_max_total_));
  jo("dispatch_phase_2_max_per_initiator", ToJson(object.dispatch_phase_2_max_per_initiator_));
  jo("dispatch_phase_3_max_per_initiator", ToJson(object.dispatch_phase_3_max_per_initiator_));
  jo("whitelist", ToJson(object.whitelist_));
  jo("prioritylist", ToJson(object.prioritylist_));
  jo("force_full_collated_data", ToJson(JsonBool{object.force_full_collated_data_}));
  jo("ignore_collated_data_limits", ToJson(JsonBool{object.ignore_collated_data_limits_}));
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_collatorsList &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.collatorsList");
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_config &object) {
  ...
  jo("dht", ToJson(object.dht_));
  jo("validators", ToJson(object.validators_));
  jo("collators", ToJson(object.collators_));
  ...
  if (object.extraconfig_) {
    jo("extraconfig", ToJson(object.extraconfig_));
  }
  jo("liteservers", ToJson(object.liteservers_));
  jo("control", ToJson(object.control_));
  jo("shards_to_monitor", ToJson(object.shards_to_monitor_));
  if (object.gc_) {
    jo("gc", ToJson(object.gc_));
  }
}
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_customOverlay &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.customOverlay");
  jo("name", ToJson(object.name_));
  jo("nodes", ToJson(object.nodes_));
  jo("sender_shards", ToJson(object.sender_shards_));
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_customOverlayNode &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.customOverlayNode");
  jo("adnl_id", ToJson(object.adnl_id_));
  jo("msg_sender", ToJson(JsonBool{object.msg_sender_}));
  jo("msg_sender_priority", ToJson(object.msg_sender_priority_));
  jo("block_sender", ToJson(JsonBool{object.block_sender_}));
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_customOverlaysConfig &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.customOverlaysConfig");
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_exportedPrivateKeys &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.exportedPrivateKeys");
  jo("encrypted_data", ToJson(JsonBytes{object.encrypted_data_}));
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_extraConfig &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.extraConfig");
  jo("state_serializer_enabled", ToJson(JsonBool{object.state_serializer_enabled_}));
  jo("fast_sync_member_certificates", ToJson(object.fast_sync_member_certificates_));
  if (object.collator_node_whitelist_) {
    jo("collator_node_whitelist", ToJson(object.collator_node_whitelist_));
  }
  jo("fast_sync_overlay_clients", ToJson(object.fast_sync_overlay_clients_));
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_fastSyncMemberCertificate &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.fastSyncMemberCertificate");
  jo("adnl_id", ToJson(object.adnl_id_));
  if (object.certificate_) {
    jo("certificate", ToJson(object.certificate_));
  }
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_fastSyncOverlayClient &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.fastSyncOverlayClient");
  jo("adnl_id", ToJson(object.adnl_id_));
  jo("slot", ToJson(object.slot_));
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_fastSyncOverlayStats &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.fastSyncOverlayStats");
  jo("shard", ToJson(object.shard_));
  jo("validators_adnl", ToJson(object.validators_adnl_));
  jo("root_public_keys", ToJson(object.root_public_keys_));
  if (object.member_certificate_) {
    jo("member_certificate", ToJson(object.member_certificate_));
  }
}
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_overlayStats &object) {
  ...
  if (object.total_traffic_) {
    jo("total_traffic", ToJson(object.total_traffic_));
  }
  if (object.total_traffic_responses_) {
    jo("total_traffic_responses", ToJson(object.total_traffic_responses_));
  }
  jo("extra", ToJson(object.extra_));
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_overlayStatsNode &object) {
  ...
  jo("is_neighbour", ToJson(JsonBool{object.is_neighbour_}));
  jo("is_alive", ToJson(JsonBool{object.is_alive_}));
  jo("node_flags", ToJson(object.node_flags_));
  ...
  if (object.traffic_) {
    jo("traffic", ToJson(object.traffic_));
  }
  if (object.traffic_responses_) {
    jo("traffic_responses", ToJson(object.traffic_responses_));
  }
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_overlayStatsTraffic &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.overlayStatsTraffic");
  jo("t_out_bytes", ToJson(JsonInt64{object.t_out_bytes_}));
  jo("t_in_bytes", ToJson(JsonInt64{object.t_in_bytes_}));
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_shardOverlayStats &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.shardOverlayStats");
  jo("shard", ToJson(object.shard_));
  jo("active", ToJson(JsonBool{object.active_}));
  jo("neighbours", ToJson(object.neighbours_));
}
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_textStats &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.textStats");
  jo("data", ToJson(object.data_));
}
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_collationManagerStats_collator &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.collationManagerStats.collator");
  jo("adnl_id", ToJson(object.adnl_id_));
  jo("active", ToJson(JsonBool{object.active_}));
  jo("alive", ToJson(JsonBool{object.alive_}));
  jo("ping_in", ToJson(object.ping_in_));
  jo("last_ping_ago", ToJson(object.last_ping_ago_));
  jo("last_ping_status", ToJson(object.last_ping_status_));
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_collationManagerStats_localId &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.collationManagerStats.localId");
  jo("adnl_id", ToJson(object.adnl_id_));
  jo("shards", ToJson(object.shards_));
  jo("collators", ToJson(object.collators_));
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_collationManagerStats_shard &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.collationManagerStats.shard");
  if (object.shard_id_) {
    jo("shard_id", ToJson(object.shard_id_));
  }
  jo("self_collate", ToJson(JsonBool{object.self_collate_}));
  jo("select_mode", ToJson(object.select_mode_));
  jo("active", ToJson(JsonBool{object.active_}));
  jo("collators", ToJson(object.collators_));
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_collatorsList_collator &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.collatorsList.collator");
  jo("adnl_id", ToJson(object.adnl_id_));
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_collatorsList_shard &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.collatorsList.shard");
  if (object.shard_id_) {
    jo("shard_id", ToJson(object.shard_id_));
  }
  jo("collators", ToJson(object.collators_));
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_shardOverlayStats_neighbour &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.shardOverlayStats.neighbour");
  jo("id", ToJson(object.id_));
  jo("verison_major", ToJson(object.verison_major_));
  jo("version_minor", ToJson(object.version_minor_));
  if (object.flags_) {
    jo("flags", ToJson(object.flags_));
  }
  jo("roundtrip", ToJson(object.roundtrip_));
  jo("unreliability", ToJson(object.unreliability_));
}
...
void to_json(JsonValueScope &jv, const ton_api::liteclient_config_global &object) {
  ...
  jo("liteservers", ToJson(object.liteservers_));
  jo("liteservers_v2", ToJson(object.liteservers_v2_));
  if (object.validator_) {
    jo("validator", ToJson(object.validator_));
  }
}
...
void to_json(JsonValueScope &jv, const ton_api::liteserver_descV2 &object) {
  auto jo = jv.enter_object();
  jo("@type", "liteserver.descV2");
  if (object.id_) {
    jo("id", ToJson(object.id_));
  }
  jo("ip", ToJson(object.ip_));
  jo("port", ToJson(object.port_));
  jo("slices", ToJson(object.slices_));
}
...
void to_json(JsonValueScope &jv, const ton_api::liteserver_descV2_shardInfo &object) {
  auto jo = jv.enter_object();
  jo("@type", "liteserver.descV2.shardInfo");
  if (object.shard_id_) {
    jo("shard_id", ToJson(object.shard_id_));
  }
  jo("seqno", ToJson(object.seqno_));
...
void to_json(JsonValueScope &jv, const ton_api::liteserver_descV2_Slice &object) {
  ton_api::downcast_call(const_cast<ton_api::liteserver_descV2_Slice &>(object), [&jv](const auto &object) { to_json(jv, object); });
}
void to_json(JsonValueScope &jv, const ton_api::liteserver_descV2_sliceSimple &object) {
  auto jo = jv.enter_object();
  jo("@type", "liteserver.descV2.sliceSimple");
  jo("shards", ToJson(object.shards_));
}
void to_json(JsonValueScope &jv, const ton_api::liteserver_descV2_sliceTimed &object) {
  auto jo = jv.enter_object();
  jo("@type", "liteserver.descV2.sliceTimed");
  jo("shards_from", ToJson(object.shards_from_));
  jo("shards_to", ToJson(object.shards_to_));
}
...
void to_json(JsonValueScope &jv, const ton_api::overlay_MemberCertificate &object) {
  ton_api::downcast_call(const_cast<ton_api::overlay_MemberCertificate &>(object), [&jv](const auto &object) { to_json(jv, object); });
}
void to_json(JsonValueScope &jv, const ton_api::overlay_memberCertificate &object) {
  auto jo = jv.enter_object();
  jo("@type", "overlay.memberCertificate");
  if (object.issued_by_) {
    jo("issued_by", ToJson(object.issued_by_));
  }
  jo("flags", ToJson(object.flags_));
...
void to_json(JsonValueScope &jv, const ton_api::overlay_emptyMemberCertificate &object) {
  auto jo = jv.enter_object();
  jo("@type", "overlay.emptyMemberCertificate");
}
void to_json(JsonValueScope &jv, const ton_api::overlay_memberCertificateId &object) {
  auto jo = jv.enter_object();
  jo("@type", "overlay.memberCertificateId");
  if (object.node_) {
    jo("node", ToJson(object.node_));
  }
  jo("flags", ToJson(object.flags_));
...
void to_json(JsonValueScope &jv, const ton_api::overlay_Message &object) {
  ton_api::downcast_call(const_cast<ton_api::overlay_Message &>(object), [&jv](const auto &object) { to_json(jv, object); });
}
void to_json(JsonValueScope &jv, const ton_api::overlay_message &object) {
  auto jo = jv.enter_object();
...
void to_json(JsonValueScope &jv, const ton_api::overlay_messageWithExtra &object) {
  auto jo = jv.enter_object();
  jo("@type", "overlay.messageWithExtra");
  jo("overlay", ToJson(object.overlay_));
  if (object.extra_) {
    jo("extra", ToJson(object.extra_));
  }
}
void to_json(JsonValueScope &jv, const ton_api::overlay_messageExtra &object) {
  auto jo = jv.enter_object();
  jo("@type", "overlay.messageExtra");
  if (object.flags_) {
    jo("flags", ToJson(object.flags_));
  }
  if (object.certificate_ && (object.flags_ & 1)) {
    jo("certificate", ToJson(object.certificate_));
  }
}
...
void to_json(JsonValueScope &jv, const ton_api::overlay_nodeV2 &object) {
  auto jo = jv.enter_object();
  jo("@type", "overlay.nodeV2");
  if (object.id_) {
    jo("id", ToJson(object.id_));
  }
  jo("overlay", ToJson(object.overlay_));
...
void to_json(JsonValueScope &jv, const ton_api::overlay_nodesV2 &object) {
  auto jo = jv.enter_object();
  jo("@type", "overlay.nodesV2");
  jo("nodes", ToJson(object.nodes_));
}
...
void to_json(JsonValueScope &jv, const ton_api::overlay_db_Nodes &object) {
  ton_api::downcast_call(const_cast<ton_api::overlay_db_Nodes &>(object), [&jv](const auto &object) { to_json(jv, object); });
}
void to_json(JsonValueScope &jv, const ton_api::overlay_db_nodesV2 &object) {
  auto jo = jv.enter_object();
  jo("@type", "overlay.db.nodesV2");
  if (object.nodes_) {
    jo("nodes", ToJson(object.nodes_));
  }
}
...
void to_json(JsonValueScope &jv, const ton_api::overlay_node_ToSign &object) {
  ton_api::downcast_call(const_cast<ton_api::overlay_node_ToSign &>(object), [&jv](const auto &object) { to_json(jv, object); });
}
void to_json(JsonValueScope &jv, const ton_api::overlay_node_toSign &object) {
  auto jo = jv.enter_object();
...
void to_json(JsonValueScope &jv, const ton_api::overlay_node_toSignEx &object) {
  auto jo = jv.enter_object();
  jo("@type", "overlay.node.toSignEx");
  if (object.id_) {
    jo("id", ToJson(object.id_));
  }
  jo("overlay", ToJson(object.overlay_));
...
void to_json(JsonValueScope &jv, const ton_api::proxyLiteserver_config &object) {
  auto jo = jv.enter_object();
  jo("@type", "proxyLiteserver.config");
  jo("port", ToJson(object.port_));
  if (object.id_) {
    jo("id", ToJson(object.id_));
  }
...
void to_json(JsonValueScope &jv, const ton_api::tonNode_blockBroadcastCompressed &object) {
  auto jo = jv.enter_object();
  jo("@type", "tonNode.blockBroadcastCompressed");
  if (object.id_) {
    jo("id", ToJson(object.id_));
  }
  jo("catchain_seqno", ToJson(object.catchain_seqno_));
  ...
  jo("compressed", ToJson(JsonBytes{object.compressed_}));
}
...
void to_json(JsonValueScope &jv, const ton_api::tonNode_newBlockCandidateBroadcast &object) {
  auto jo = jv.enter_object();
  jo("@type", "tonNode.newBlockCandidateBroadcast");
  if (object.id_) {
    jo("id", ToJson(object.id_));
  }
  jo("catchain_seqno", ToJson(object.catchain_seqno_));
  jo("validator_set_hash", ToJson(object.validator_set_hash_));
  if (object.collator_signature_) {
    jo("collator_signature", ToJson(object.collator_signature_));
  }
  jo("data", ToJson(JsonBytes{object.data_}));
}
void to_json(JsonValueScope &jv, const ton_api::tonNode_newBlockCandidateBroadcastCompressed &object) {
  auto jo = jv.enter_object();
  jo("@type", "tonNode.newBlockCandidateBroadcastCompressed");
  if (object.id_) {
    jo("id", ToJson(object.id_));
  }
  jo("catchain_seqno", ToJson(object.catchain_seqno_));
...
void to_json(JsonValueScope &jv, const ton_api::tonNode_outMsgQueueProofBroadcast &object) {
  auto jo = jv.enter_object();
  jo("@type", "tonNode.outMsgQueueProofBroadcast");
  if (object.dst_shard_) {
    jo("dst_shard", ToJson(object.dst_shard_));
  }
  if (object.block_) {
    jo("block", ToJson(object.block_));
  }
  ...
  if (object.proof_) {
    jo("proof", ToJson(object.proof_));
  }
}
void to_json(JsonValueScope &jv, const ton_api::tonNode_capabilities &object) {
  auto jo = jv.enter_object();
  jo("@type", "tonNode.capabilities");
  jo("version_major", ToJson(object.version_major_));
  jo("version_minor", ToJson(object.version_minor_));
  if (object.flags_) {
    jo("flags", ToJson(object.flags_));
  }
}
void to_json(JsonValueScope &jv, const ton_api::tonNode_customOverlayId &object) {
  auto jo = jv.enter_object();
  jo("@type", "tonNode.customOverlayId");
...
void to_json(JsonValueScope &jv, const ton_api::tonNode_dataFullCompressed &object) {
  auto jo = jv.enter_object();
  jo("@type", "tonNode.dataFullCompressed");
  if (object.id_) {
    jo("id", ToJson(object.id_));
  }
  if (object.flags_) {
...
void to_json(JsonValueScope &jv, const ton_api::tonNode_fastSyncOverlayId &object) {
  auto jo = jv.enter_object();
  jo("@type", "tonNode.fastSyncOverlayId");
  jo("zero_state_file_hash", ToJson(object.zero_state_file_hash_));
  if (object.shard_) {
    jo("shard", ToJson(object.shard_));
  }
}
void to_json(JsonValueScope &jv, const ton_api::tonNode_forgetPeer &object) {
  auto jo = jv.enter_object();
  jo("@type", "tonNode.forgetPeer");
}
...
void to_json(JsonValueScope &jv, const ton_api::tonNode_newFastSyncMemberCertificate &object) {
  auto jo = jv.enter_object();
  jo("@type", "tonNode.newFastSyncMemberCertificate");
  jo("adnl_id", ToJson(object.adnl_id_));
  if (object.certificate_) {
    jo("certificate", ToJson(object.certificate_));
  }
...
void to_json(JsonValueScope &jv, const ton_api::tonNode_OutMsgQueueProof &object) {
  ton_api::downcast_call(const_cast<ton_api::tonNode_OutMsgQueueProof &>(object), [&jv](const auto &object) { to_json(jv, object); });
}
void to_json(JsonValueScope &jv, const ton_api::tonNode_outMsgQueueProof &object) {
  auto jo = jv.enter_object();
  jo("@type", "tonNode.outMsgQueueProof");
  jo("queue_proofs", ToJson(JsonBytes{object.queue_proofs_}));
  jo("block_state_proofs", ToJson(JsonBytes{object.block_state_proofs_}));
  jo("msg_counts", ToJson(object.msg_counts_));
}
void to_json(JsonValueScope &jv, const ton_api::tonNode_outMsgQueueProofEmpty &object) {
  auto jo = jv.enter_object();
  jo("@type", "tonNode.outMsgQueueProofEmpty");
}
...
void to_json(JsonValueScope &jv, const ton_api::tonNode_shardId &object) {
  auto jo = jv.enter_object();
  jo("@type", "tonNode.shardId");
  jo("workchain", ToJson(object.workchain_));
  jo("shard", ToJson(JsonInt64{object.shard_}));
}
...
void to_json(JsonValueScope &jv, const ton_api::tonNode_blockBroadcastCompressed_data &object) {
  auto jo = jv.enter_object();
  jo("@type", "tonNode.blockBroadcastCompressed.data");
  jo("signatures", ToJson(object.signatures_));
  jo("proof_data", ToJson(JsonBytes{object.proof_data_}));
}
...
void to_json(JsonValueScope &jv, const ton_api::validator_telemetry &object) {
  auto jo = jv.enter_object();
  jo("@type", "validator.telemetry");
  if (object.flags_) {
    jo("flags", ToJson(object.flags_));
  }
  jo("timestamp", ToJson(object.timestamp_));
  jo("adnl_id", ToJson(object.adnl_id_));
  jo("node_version", ToJson(object.node_version_));
  jo("os_version", ToJson(object.os_version_));
  jo("node_started_at", ToJson(object.node_started_at_));
  jo("ram_size", ToJson(JsonInt64{object.ram_size_}));
  jo("cpu_cores", ToJson(object.cpu_cores_));
  jo("node_threads", ToJson(object.node_threads_));
}
...
void to_json(JsonValueScope &jv, const ton_api::validatorSession_Candidate &object) {
  ton_api::downcast_call(const_cast<ton_api::validatorSession_Candidate &>(object), [&jv](const auto &object) { to_json(jv, object); });
}
void to_json(JsonValueScope &jv, const ton_api::validatorSession_candidate &object) {
  auto jo = jv.enter_object();
...
void to_json(JsonValueScope &jv, const ton_api::validatorSession_compressedCandidate &object) {
  auto jo = jv.enter_object();
  jo("@type", "validatorSession.compressedCandidate");
  if (object.flags_) {
    jo("flags", ToJson(object.flags_));
  }
  jo("src", ToJson(object.src_));
...
void to_json(JsonValueScope &jv, const ton_api::validatorStats_collatedBlock &object) {
  auto jo = jv.enter_object();
  jo("@type", "validatorStats.collatedBlock");
  if (object.block_id_) {
    jo("block_id", ToJson(object.block_id_));
  }
  jo("collated_data_hash", ToJson(object.collated_data_hash_));
  jo("cc_seqno", ToJson(object.cc_seqno_));
  jo("collated_at", ToJson(object.collated_at_));
  jo("bytes", ToJson(object.bytes_));
  jo("collated_data_bytes", ToJson(object.collated_data_bytes_));
  jo("attempt", ToJson(object.attempt_));
  jo("self", ToJson(object.self_));
  jo("is_validator", ToJson(JsonBool{object.is_validator_}));
  jo("total_time", ToJson(object.total_time_));
  jo("work_time", ToJson(object.work_time_));
  jo("cpu_work_time", ToJson(object.cpu_work_time_));
  jo("time_stats", ToJson(object.time_stats_));
  if (object.block_limits_) {
    jo("block_limits", ToJson(object.block_limits_));
  }
  if (object.ext_msgs_stats_) {
    jo("ext_msgs_stats", ToJson(object.ext_msgs_stats_));
  }
}
...
void to_json(JsonValueScope &jv, const ton_api::validatorStats_blockLimitsStatus &object) {
  auto jo = jv.enter_object();
  jo("@type", "validatorStats.blockLimitsStatus");
  jo("bytes", ToJson(object.bytes_));
  jo("gas", ToJson(object.gas_));
  jo("lt_delta", ToJson(object.lt_delta_));
  jo("collated_data_bytes", ToJson(object.collated_data_bytes_));
  jo("cat_bytes", ToJson(object.cat_bytes_));
  jo("cat_gas", ToJson(object.cat_gas_));
  jo("cat_lt_delta", ToJson(object.cat_lt_delta_));
  jo("cat_collated_data_bytes", ToJson(object.cat_collated_data_bytes_));
  jo("limits_log", ToJson(object.limits_log_));
}
void to_json(JsonValueScope &jv, const ton_api::validatorStats_collatorNodeResponse &object) {
  auto jo = jv.enter_object();
  jo("@type", "validatorStats.collatorNodeResponse");
  jo("self", ToJson(object.self_));
  jo("validator_id", ToJson(object.validator_id_));
  jo("timestamp", ToJson(object.timestamp_));
  if (object.block_id_) {
    jo("block_id", ToJson(object.block_id_));
  }
  if (object.original_block_id_) {
    jo("original_block_id", ToJson(object.original_block_id_));
  }
  jo("collated_data_hash", ToJson(object.collated_data_hash_));
}
void to_json(JsonValueScope &jv, const ton_api::validatorStats_endValidatorGroup &object) {
  auto jo = jv.enter_object();
  jo("@type", "validatorStats.endValidatorGroup");
  jo("session_id", ToJson(object.session_id_));
  jo("timestamp", ToJson(object.timestamp_));
  jo("self", ToJson(object.self_));
  jo("nodes", ToJson(object.nodes_));
}
void to_json(JsonValueScope &jv, const ton_api::validatorStats_extMsgsStats &object) {
  auto jo = jv.enter_object();
  jo("@type", "validatorStats.extMsgsStats");
  jo("ext_msgs_total", ToJson(object.ext_msgs_total_));
  jo("ext_msgs_filtered", ToJson(object.ext_msgs_filtered_));
  jo("ext_msgs_accepted", ToJson(object.ext_msgs_accepted_));
  jo("ext_msgs_rejected", ToJson(object.ext_msgs_rejected_));
}
void to_json(JsonValueScope &jv, const ton_api::validatorStats_newValidatorGroup &object) {
  auto jo = jv.enter_object();
  jo("@type", "validatorStats.newValidatorGroup");
  jo("session_id", ToJson(object.session_id_));
  if (object.shard_) {
    jo("shard", ToJson(object.shard_));
  }
  jo("cc_seqno", ToJson(object.cc_seqno_));
  jo("last_key_block_seqno", ToJson(object.last_key_block_seqno_));
  jo("started_at", ToJson(object.started_at_));
  jo("self_idx", ToJson(object.self_idx_));
  jo("self", ToJson(object.self_));
  jo("nodes", ToJson(object.nodes_));
}
void to_json(JsonValueScope &jv, const ton_api::validatorStats_stats &object) {
  auto jo = jv.enter_object();
  jo("@type", "validatorStats.stats");
  if (object.flags_) {
    jo("flags", ToJson(object.flags_));
  }
  jo("session_id", ToJson(object.session_id_));
  ...
  jo("cc_seqno", ToJson(object.cc_seqno_));
  jo("success", ToJson(JsonBool{object.success_}));
  jo("timestamp", ToJson(object.timestamp_));
  if ((object.flags_ & 1)) {
...
void to_json(JsonValueScope &jv, const ton_api::validatorStats_validatedBlock &object) {
  auto jo = jv.enter_object();
  jo("@type", "validatorStats.validatedBlock");
  if (object.block_id_) {
    jo("block_id", ToJson(object.block_id_));
  }
  jo("collated_data_hash", ToJson(object.collated_data_hash_));
  jo("validated_at", ToJson(object.validated_at_));
  jo("self", ToJson(object.self_));
  jo("valid", ToJson(JsonBool{object.valid_}));
  jo("comment", ToJson(object.comment_));
  jo("bytes", ToJson(object.bytes_));
  jo("collated_data_bytes", ToJson(object.collated_data_bytes_));
  jo("total_time", ToJson(object.total_time_));
  jo("work_time", ToJson(object.work_time_));
...
void to_json(JsonValueScope &jv, const ton_api::validatorStats_endValidatorGroup_node &object) {
  auto jo = jv.enter_object();
  jo("@type", "validatorStats.endValidatorGroup.node");
  jo("id", ToJson(object.id_));
  jo("catchain_blocks", ToJson(object.catchain_blocks_));
}
void to_json(JsonValueScope &jv, const ton_api::validatorStats_newValidatorGroup_node &object) {
  auto jo = jv.enter_object();
  jo("@type", "validatorStats.newValidatorGroup.node");
  jo("id", ToJson(object.id_));
  if (object.pubkey_) {
    jo("pubkey", ToJson(object.pubkey_));
  }
...
void to_json(JsonValueScope &jv, const ton_api::validatorStats_stats_producer &object) {
  auto jo = jv.enter_object();
  jo("@type", "validatorStats.stats.producer");
  if (object.flags_) {
    jo("flags", ToJson(object.flags_));
  }
  jo("validator_id", ToJson(object.validator_id_));
  ...
  if ((object.flags_ & 1)) {
    jo("candidate_id", ToJson(object.candidate_id_));
  }
  if (object.block_id_ && (object.flags_ & 1)) {
    jo("block_id", ToJson(object.block_id_));
  }
  if ((object.flags_ & 1)) {
    jo("collated_data_hash", ToJson(object.collated_data_hash_));
  ...
  if ((object.flags_ & 1)) {
    jo("is_accepted", ToJson(JsonBool{object.is_accepted_}));
  }
  if ((object.flags_ & 1)) {
    jo("is_ours", ToJson(JsonBool{object.is_ours_}));
  }
  if ((object.flags_ & 1)) {
    jo("got_block_at", ToJson(object.got_block_at_));
  }
  if ((object.flags_ & 1)) {
    jo("got_submit_at", ToJson(object.got_submit_at_));
  }
  if ((object.flags_ & 1)) {
  ...
  if ((object.flags_ & 2)) {
    jo("collation_time", ToJson(object.collation_time_));
  }
  if ((object.flags_ & 2)) {
    jo("collated_at", ToJson(object.collated_at_));
  }
  if ((object.flags_ & 2)) {
    jo("collation_cached", ToJson(JsonBool{object.collation_cached_}));
  }
  if ((object.flags_ & 2)) {
    jo("self_collated", ToJson(JsonBool{object.self_collated_}));
  }
  if ((object.flags_ & 4)) {
    jo("collator_node_id", ToJson(object.collator_node_id_));
  }
  if ((object.flags_ & 8)) {
    jo("validation_time", ToJson(object.validation_time_));
  }
  if ((object.flags_ & 8)) {
    jo("validated_at", ToJson(object.validated_at_));
  }
  if ((object.flags_ & 8)) {
    jo("validation_cached", ToJson(JsonBool{object.validation_cached_}));
  }
  if ((object.flags_ & 1)) {
    jo("approved_weight", ToJson(JsonInt64{object.approved_weight_}));
  }
  if ((object.flags_ & 1)) {
    jo("approved_33pct_at", ToJson(object.approved_33pct_at_));
  }
  if ((object.flags_ & 1)) {
    jo("approved_66pct_at", ToJson(object.approved_66pct_at_));
  }
  if ((object.flags_ & 1)) {
    jo("approvers", ToJson(object.approvers_));
  }
  if ((object.flags_ & 1)) {
    jo("signed_weight", ToJson(JsonInt64{object.signed_weight_}));
  }
  if ((object.flags_ & 1)) {
    jo("signed_33pct_at", ToJson(object.signed_33pct_at_));
  }
  if ((object.flags_ & 1)) {
    jo("signed_66pct_at", ToJson(object.signed_66pct_at_));
  }
  if ((object.flags_ & 1)) {
    jo("signers", ToJson(object.signers_));
  }
  if ((object.flags_ & 16)) {
    jo("serialize_time", ToJson(object.serialize_time_));
  }
  if ((object.flags_ & 16)) {
    jo("deserialize_time", ToJson(object.deserialize_time_));
  }
  if ((object.flags_ & 16)) {
    jo("serialized_size", ToJson(object.serialized_size_));
  }
}
void to_json(JsonValueScope &jv, const ton_api::validatorStats_stats_round &object) {
  auto jo = jv.enter_object();
  jo("@type", "validatorStats.stats.round");
  jo("started_at", ToJson(object.started_at_));
...
void to_json(JsonValueScope &jv, const ton_api::collatorNode_generateBlock &object) {
  auto jo = jv.enter_object();
  jo("@type", "collatorNode.generateBlock");
  if (object.shard_) {
    jo("shard", ToJson(object.shard_));
  }
  jo("cc_seqno", ToJson(object.cc_seqno_));
  ...
  jo("creator", ToJson(object.creator_));
  jo("round", ToJson(object.round_));
  jo("first_block_round", ToJson(object.first_block_round_));
  jo("priority", ToJson(object.priority_));
}
void to_json(JsonValueScope &jv, const ton_api::collatorNode_ping &object) {
  auto jo = jv.enter_object();
  jo("@type", "collatorNode.ping");
  if (object.flags_) {
    jo("flags", ToJson(object.flags_));
  }
}
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_addCollator &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.addCollator");
  jo("adnl_id", ToJson(object.adnl_id_));
  if (object.shard_) {
    jo("shard", ToJson(object.shard_));
  }
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_addCustomOverlay &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.addCustomOverlay");
  if (object.overlay_) {
    jo("overlay", ToJson(object.overlay_));
  }
}
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_addFastSyncClient &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.addFastSyncClient");
  jo("adnl_id", ToJson(object.adnl_id_));
  jo("slot", ToJson(object.slot_));
}
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_addShard &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.addShard");
  if (object.shard_) {
    jo("shard", ToJson(object.shard_));
  }
}
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_clearCollatorsList &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.clearCollatorsList");
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_collatorNodeSetWhitelistEnabled &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.collatorNodeSetWhitelistEnabled");
  jo("enabled", ToJson(JsonBool{object.enabled_}));
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_collatorNodeSetWhitelistedValidator &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.collatorNodeSetWhitelistedValidator");
  jo("adnl_id", ToJson(object.adnl_id_));
  jo("add", ToJson(JsonBool{object.add_}));
}
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_delCollator &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.delCollator");
  jo("adnl_id", ToJson(object.adnl_id_));
  if (object.shard_) {
    jo("shard", ToJson(object.shard_));
  }
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_delCustomOverlay &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.delCustomOverlay");
  jo("name", ToJson(object.name_));
}
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_delFastSyncClient &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.delFastSyncClient");
  jo("adnl_id", ToJson(object.adnl_id_));
}
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_delShard &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.delShard");
  if (object.shard_) {
    jo("shard", ToJson(object.shard_));
  }
}
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_exportAllPrivateKeys &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.exportAllPrivateKeys");
  if (object.encryption_key_) {
    jo("encryption_key", ToJson(object.encryption_key_));
  }
}
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_getActorTextStats &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.getActorTextStats");
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_getAdnlStats &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.getAdnlStats");
  jo("all", ToJson(JsonBool{object.all_}));
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_getCollationManagerStats &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.getCollationManagerStats");
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_getCollatorOptionsJson &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.getCollatorOptionsJson");
}
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_importFastSyncMemberCertificate &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.importFastSyncMemberCertificate");
  jo("adnl_id", ToJson(object.adnl_id_));
  if (object.certificate_) {
    jo("certificate", ToJson(object.certificate_));
  }
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_setCollatorOptionsJson &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.setCollatorOptionsJson");
  jo("json", ToJson(object.json_));
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_setCollatorsList &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.setCollatorsList");
  if (object.list_) {
    jo("list", ToJson(object.list_));
  }
}
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_setStateSerializerEnabled &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.setStateSerializerEnabled");
  jo("enabled", ToJson(JsonBool{object.enabled_}));
}
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_showCollatorNodeWhitelist &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.showCollatorNodeWhitelist");
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_showCollatorsList &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.showCollatorsList");
}
void to_json(JsonValueScope &jv, const ton_api::engine_validator_showCustomOverlays &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.showCustomOverlays");
}
...
void to_json(JsonValueScope &jv, const ton_api::engine_validator_signOverlayMemberCertificate &object) {
  auto jo = jv.enter_object();
  jo("@type", "engine.validator.signOverlayMemberCertificate");
  jo("sign_by", ToJson(object.sign_by_));
  jo("adnl_id", ToJson(object.adnl_id_));
  jo("slot", ToJson(object.slot_));
  jo("expire_at", ToJson(object.expire_at_));
}
...
void to_json(JsonValueScope &jv, const ton_api::overlay_getRandomPeersV2 &object) {
  auto jo = jv.enter_object();
  jo("@type", "overlay.getRandomPeersV2");
  if (object.peers_) {
    jo("peers", ToJson(object.peers_));
  }
}
...
void to_json(JsonValueScope &jv, const ton_api::overlay_queryWithExtra &object) {
  auto jo = jv.enter_object();
  jo("@type", "overlay.queryWithExtra");
  jo("overlay", ToJson(object.overlay_));
  if (object.extra_) {
    jo("extra", ToJson(object.extra_));
  }
...
void to_json(JsonValueScope &jv, const ton_api::tonNode_getOutMsgQueueProof &object) {
  auto jo = jv.enter_object();
  jo("@type", "tonNode.getOutMsgQueueProof");
  if (object.dst_shard_) {
    jo("dst_shard", ToJson(object.dst_shard_));
  }
  jo("blocks", ToJson(object.blocks_));
...
void to_json(JsonValueScope &jv, const ton_api::tonNode_getShardArchiveInfo &object) {
  auto jo = jv.enter_object();
  jo("@type", "tonNode.getShardArchiveInfo");
  jo("masterchain_seqno", ToJson(object.masterchain_seqno_));
  if (object.shard_prefix_) {
    jo("shard_prefix", ToJson(object.shard_prefix_));
  }
}
...
void to_json(JsonValueScope &jv, const ton_api::tonNode_requestFastSyncOverlayMemberCertificate &object) {
  auto jo = jv.enter_object();
  jo("@type", "tonNode.requestFastSyncOverlayMemberCertificate");
  jo("sign_by", ToJson(object.sign_by_));
  jo("adnl_id", ToJson(object.adnl_id_));
  jo("slot", ToJson(object.slot_));
}
