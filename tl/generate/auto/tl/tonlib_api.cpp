...
extraCurrency::extraCurrency(std::int32_t id_, std::int64_t amount_)
  : id_(id_)
  , amount_(amount_)
{}

const std::int32_t extraCurrency::ID;

void extraCurrency::store(td::TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    s.store_class_begin(field_name, "extraCurrency");
...
fullAccountState::fullAccountState(object_ptr<accountAddress> &&address_, std::int64_t balance_, std::vector<object_ptr<extraCurrency>> &&extra_currencies_, object_ptr<internal_transactionId> &&last_transaction_id_, object_ptr<ton_blockIdExt> &&block_id_, std::int64_t sync_utime_, object_ptr<AccountState> &&account_state_, std::int32_t revision_)
  : address_(std::move(address_))
  , balance_(balance_)
  , extra_currencies_(std::move(extra_currencies_))
...
void fullAccountState::store(td::TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<extraCurrency>> &v = extra_currencies_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + td::to_string(multiplicity)+ "]"; s.store_class_begin("extra_currencies", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
...
blocks_outMsgQueueSize::blocks_outMsgQueueSize()
  : id_()
  , size_()
{}

blocks_outMsgQueueSize::blocks_outMsgQueueSize(object_ptr<ton_blockIdExt> &&id_, std::int32_t size_)
  : id_(std::move(id_))
  , size_(size_)
{}

const std::int32_t blocks_outMsgQueueSize::ID;

void blocks_outMsgQueueSize::store(td::TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    s.store_class_begin(field_name, "blocks_outMsgQueueSize");
    if (id_ == nullptr) { s.store_field("id", "null"); } else { id_->store(s, "id"); }
...
blocks_outMsgQueueSizes::blocks_outMsgQueueSizes()
  : shards_()
  , ext_msg_queue_size_limit_()
{}

blocks_outMsgQueueSizes::blocks_outMsgQueueSizes(std::vector<object_ptr<blocks_outMsgQueueSize>> &&shards_, std::int32_t ext_msg_queue_size_limit_)
  : shards_(std::move(shards_))
  , ext_msg_queue_size_limit_(ext_msg_queue_size_limit_)
{}

const std::int32_t blocks_outMsgQueueSizes::ID;

void blocks_outMsgQueueSizes::store(td::TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    s.store_class_begin(field_name, "blocks_outMsgQueueSizes");
    { const std::vector<object_ptr<blocks_outMsgQueueSize>> &v = shards_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + td::to_string(multiplicity)+ "]"; s.store_class_begin("shards", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
...
msg_message::msg_message(object_ptr<accountAddress> &&destination_, std::string const &public_key_, std::int64_t amount_, std::vector<object_ptr<extraCurrency>> &&extra_currencies_, object_ptr<msg_Data> &&data_, std::int32_t send_mode_)
...
raw_fullAccountState::raw_fullAccountState(std::int64_t balance_, std::vector<object_ptr<extraCurrency>> &&extra_currencies_, std::string const &code_, std::string const &data_, object_ptr<internal_transactionId> &&last_transaction_id_, object_ptr<ton_blockIdExt> &&block_id_, std::string const &frozen_hash_, std::int64_t sync_utime_)
  : balance_(balance_)
  , extra_currencies_(std::move(extra_currencies_))
  , code_(std::move(code_))
  , data_(std::move(data_))
...
raw_message::raw_message(std::string const &hash_, object_ptr<accountAddress> &&source_, object_ptr<accountAddress> &&destination_, std::int64_t value_, std::vector<object_ptr<extraCurrency>> &&extra_currencies_, std::int64_t fwd_fee_, std::int64_t ihr_fee_, std::int64_t created_lt_, std::string const &body_hash_, object_ptr<msg_Data> &&msg_data_)
  : hash_(std::move(hash_))
  , source_(std::move(source_))
  , destination_(std::move(destination_))
  , value_(value_)
...
blocks_getOutMsgQueueSizes::blocks_getOutMsgQueueSizes()
  : mode_()
  , wc_()
  , shard_()
{}

blocks_getOutMsgQueueSizes::blocks_getOutMsgQueueSizes(std::int32_t mode_, std::int32_t wc_, std::int64_t shard_)
  : mode_(mode_)
  , wc_(wc_)
  , shard_(shard_)
{}

const std::int32_t blocks_getOutMsgQueueSizes::ID;

void blocks_getOutMsgQueueSizes::store(td::TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    s.store_class_begin(field_name, "blocks_getOutMsgQueueSizes");
    s.store_field("mode", (var0 = mode_));
...
smc_getRawFullAccountState::smc_getRawFullAccountState()
  : id_()
{}

smc_getRawFullAccountState::smc_getRawFullAccountState(std::int64_t id_)
  : id_(id_)
{}

const std::int32_t smc_getRawFullAccountState::ID;

void smc_getRawFullAccountState::store(td::TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    s.store_class_begin(field_name, "smc_getRawFullAccountState");
    s.store_field("id", id_);
    s.store_class_end();
  }
}
