...
std::vector<GroupIdentity> identities_for(const Context &ctx, const td::Ref<block::ValidatorSet> &val_set,
  ...
  for (auto &n : val_set->export_vector()) {
    ...
    for (auto collator : ctx.collators_by_validator.at(key)) {
      if (ctx.local_collator_ids.contains(collator)) {
        CHECK(!group_validator_adnl_ids.contains(collator));
        identities.push_back({
            .adnl_id = collator,
            .is_collator = true,
        });
...
SessionInfo session_info(const Context &ctx, ShardIdFull shard, td::Ref<block::ValidatorSet> validator_set) {
  ...
  for (auto &identity : identities_for(ctx, validator_set, config)) {
    bool observers_wanted = config.enable_block_sync() || config.observers_in_private_overlay();
    bool collators_wanted = config.enable_collators();
    if (identity.is_validator() || (identity.is_observer && observers_wanted) ||
        (identity.is_collator && collators_wanted)) {
