...
class RunFileSink : public CellSink {
 public:
  ...
  void emit(const Ref<vm::DataCell> &cell) override {
    ...
    Rec rec;
    rec.hash = td::Bits256{cell->get_hash().bits()};
...
std::pair<Ref<vm::Cell>, Uint128> build_account_cells(const GenContext &ctx, const DeriveRecord &rec, CellSink &sink) {
  ...
  switch (static_cast<AccountType>(rec.type)) {
    case AccountType::W5:
      data = emit_and_standin(sink, build_w5_data(payload, cfg.wallet_id));
    ...
    case AccountType::JW:
      data =
          emit_and_standin(sink, build_jw_data(cfg.jw_jetton_balance, payload, ctx.minter_addr, ctx.jw_code_standin));
    ...
    case AccountType::Minter:
      data = emit_and_standin(sink, build_minter_data(cfg.total_supply(), ctx.empty_cell_standin, ctx.jw_code_standin));
  ...
  return {emit_and_standin(sink, account), balance};
}
...
MergeStats merge_runs_to_sst(const std::vector<std::string> &runs, const std::string &sst_dir,
  ...
  while (!heap.empty()) {
    ...
    if (have_prev && cur.hash() == prev_hash) {
      // duplicate: identical hash must carry identical value
...
void self_test_celldb(const Config &base_cfg) {
  Config cfg = base_cfg;
  cfg.num_v5 = 2000;
  cfg.num_ballast = 100;
