...
class RldpConnectionActor : public td::actor::Actor, private ConnectionCallback {
 ...
 private:
  ...
  void tear_down() override {
    ...
    send_closure(rldp_, &RldpIn::absorb, connection_.drain(),
...
td::actor::Task<> RldpIn::collect(metrics::Context ctx) {
  ...
  using DrainTask = decltype(td::actor::ask(td::actor::ActorId<RldpConnectionActor>{},
                                            &RldpConnectionActor::collect_metrics));
  ...
  metrics_.transport->connections.set(static_cast<double>(connections_.size()));
  metrics_.transport->queries_pending.set(static_cast<double>(queries_.size()));
  metrics_.transport->dropped = aggregate_.transport_dropped;

  auto rldp = ctx.with_name("rldp2");
  rldp.collect(aggregate_.wire);
