...
struct RldpConnMetrics {
  struct Wire {
    metrics::Labeled<metrics::Counter<"adnl_bytes">, metrics::Direction> bytes;
    metrics::Labeled<metrics::Counter<"adnl_messages">, metrics::Direction> messages;
    ...
    void collect(td::Badge<metrics::Context>, metrics::Context ctx) const {
      ctx.collect(bytes);
      ctx.collect(messages);
    }
  };

  metrics::Prefixed<"wire", Wire> wire;
  metrics::Labeled<metrics::Counter<"dropped">, metrics::Direction, metrics::Reason> transport_dropped;
